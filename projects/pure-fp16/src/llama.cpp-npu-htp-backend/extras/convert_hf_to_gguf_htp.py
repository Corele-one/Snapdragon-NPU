...
if 'NO_LOCAL_GGUF' not in os.environ:
    # Deploy-v1 may run this script from extras/ while gguf-py lives at the
    # llama.cpp tree root. Keep the historical extras/gguf-py probe, then add
    # the actual local checkout path so remote export does not require pip gguf.
    sys.path.insert(1, str(Path(__file__).parent / 'gguf-py'))
    sys.path.insert(1, str(Path(__file__).resolve().parents[1] / 'gguf-py'))
import gguf
...
class Model:
    ...
    def _is_htp_matmul_tensor(self, new_name: str, bid: int | None) -> bool:
        return any(self.match_model_tensor_name(new_name, key, bid) for key in [
    ...
    def _use_w8pc_a8pt_convert(self) -> bool:
        value = os.environ.get("LLAMA_NPU_W8PC_A8PT_CONVERT", "")
        return value.lower() in {"1", "true", "yes", "on"}

    def _use_lpbq_a8w8_convert(self) -> bool:
        value = os.environ.get("LLAMA_NPU_LPBQ_A8W8_CONVERT", "")
        return value.lower() in {"1", "true", "yes", "on"}

    def _lpbq_group_size(self) -> int:
        value = int(os.environ.get("LLAMA_NPU_LPBQ_GROUP_SIZE", "16"))
        if value not in (16, 32):
            raise ValueError(f"LPBQ deploy-v1 supports group size 16 or 32, got {value}")
        return value

    def _lpbq_sidecar_dir(self) -> Path:
        value = os.environ.get("LLAMA_NPU_LPBQ_SIDECAR_DIR", "")
        if not value:
            raise ValueError("LLAMA_NPU_LPBQ_SIDECAR_DIR must be set when LLAMA_NPU_LPBQ_A8W8_CONVERT=1")
        out = Path(value)
        out.mkdir(parents=True, exist_ok=True)
        return out

    @staticmethod
    def _lpbq_sidecar_stem(name: str) -> str:
        return "".join(ch if ch.isalnum() or ch in "._-" else "_" for ch in name) or "unnamed"

    def _lpbq_act_scale_map(self) -> dict[str, float]:
        cache = getattr(self, "_lpbq_act_scale_cache", None)
        if cache is not None:
            return cache
        path = os.environ.get("LLAMA_NPU_LPBQ_ACT_SCALE_JSON", "")
        values: dict[str, float] = {}
        if path:
            with open(path, "r", encoding="utf-8") as f:
                raw = json.load(f)
            if isinstance(raw, dict) and isinstance(raw.get("act_scales"), dict):
                raw = raw["act_scales"]
            if not isinstance(raw, dict):
                raise ValueError("LLAMA_NPU_LPBQ_ACT_SCALE_JSON must contain an object or {'act_scales': object}")
            values = {str(k): float(v) for k, v in raw.items()}
        setattr(self, "_lpbq_act_scale_cache", values)
        return values

    def _lpbq_lookup_act_scale(self, source_name: str, new_name: str) -> float:
        values = self._lpbq_act_scale_map()
        keys = [
            new_name,
            new_name.removesuffix(".weight"),
            source_name,
            source_name.removesuffix(".weight"),
        ]
        for key in keys:
            if key in values:
                scale = float(values[key])
                if scale <= 0 or not math.isfinite(scale):
                    raise ValueError(f"invalid LPBQ act_scale for {key}: {scale}")
                return scale
        default = os.environ.get("LLAMA_NPU_LPBQ_ACT_SCALE_DEFAULT", "")
        if default:
            scale = float(default)
            if scale <= 0 or not math.isfinite(scale):
                raise ValueError(f"invalid LLAMA_NPU_LPBQ_ACT_SCALE_DEFAULT={default}")
            return scale
        raise ValueError(
            f"missing LPBQ act_scale for {new_name}; provide LLAMA_NPU_LPBQ_ACT_SCALE_JSON from the folded QAT checkpoint"
        )

    def _lpbq_input_scale_map(self) -> dict[str, list[float]]:
        cache = getattr(self, "_lpbq_input_scale_cache", None)
        if cache is not None:
            return cache
        path = os.environ.get("LLAMA_NPU_LPBQ_INPUT_SCALE_JSON", "")
        values: dict[str, list[float]] = {}
        if path:
            with open(path, "r", encoding="utf-8") as f:
                raw = json.load(f)
            if isinstance(raw, dict) and isinstance(raw.get("input_scales"), dict):
                raw = raw["input_scales"]
            if not isinstance(raw, dict):
                raise ValueError("LLAMA_NPU_LPBQ_INPUT_SCALE_JSON must contain an object or {'input_scales': object}")
            for key, value in raw.items():
                if not isinstance(value, list):
                    raise ValueError(f"LPBQ input_scale for {key} must be a list")
                values[str(key)] = [float(x) for x in value]
        setattr(self, "_lpbq_input_scale_cache", values)
        return values

    def _lpbq_lookup_input_scale(self, source_name: str, new_name: str, k: int) -> np.ndarray | None:
        values = self._lpbq_input_scale_map()
        keys = [
            new_name,
            new_name.removesuffix(".weight"),
            source_name,
        ...
        for key in keys:
            if key not in values:
                continue
            arr = np.asarray(values[key], dtype=np.float32).reshape(-1)
            if arr.size != k:
                raise ValueError(f"LPBQ input_scale for {key} has {arr.size} values, expected K={k}")
            if not np.all(np.isfinite(arr)):
                raise ValueError(f"LPBQ input_scale for {key} contains non-finite values")
            return arr
        return None

    def _quantize_lpbq_a8w8_bytes(self, data_torch: Tensor, source_name: str, new_name: str) -> np.ndarray:
        data = data_torch.detach().cpu().to(torch.float32).numpy()
        if data.ndim != 2:
            raise ValueError("LPBQ_A8W8 conversion expects a 2D tensor")

        n, k = data.shape
        if n % 32 != 0 or k % 32 != 0:
            raise ValueError(f"LPBQ_A8W8 tensor shape must be 32-aligned, got {(n, k)}")

        group_size = self._lpbq_group_size()
        if k % group_size != 0:
            raise ValueError(f"LPBQ_A8W8 K={k} must be divisible by group size {group_size}")

        grouped = data.reshape(n, k // group_size, group_size)
        block_scale = np.maximum(np.max(np.abs(grouped), axis=2, keepdims=True) / np.float32(7.0), np.float32(1e-12))
        scale2 = np.maximum(np.max(block_scale, axis=1, keepdims=True) / np.float32(16.0), np.float32(1e-12))
        scale1 = np.clip(np.rint(block_scale / scale2), 1, 16).astype(np.int16)
        q4 = np.clip(np.rint(grouped / block_scale), -7, 7).astype(np.int16)
        expanded = np.clip(q4 * scale1, -128, 127).astype(np.int8).reshape(n, k)

        sidecar_dir = self._lpbq_sidecar_dir()
        stem = self._lpbq_sidecar_stem(new_name)
        scale2.astype(np.float32).reshape(n).tofile(sidecar_dir / f"{stem}.lpbq_scale2.f32")
        np.array([self._lpbq_lookup_act_scale(source_name, new_name)], dtype=np.float32).tofile(
            sidecar_dir / f"{stem}.lpbq_act_scale.f32"
        )
        input_scale = self._lpbq_lookup_input_scale(source_name, new_name, k)
        if input_scale is not None:
            input_scale.astype(np.float32).tofile(sidecar_dir / f"{stem}.lpbq_input_scale.f32")

        blocks = np.empty((n // 32 * k // 32 * 32, 34), dtype=np.uint8)
        one = np.array([1.0], dtype=np.float16).view(np.uint8)
        block_idx = 0
        for n_tile in range(n // 32):
            for k_tile in range(k // 32):
                ...
                for c in range(32):
                    row = n_tile * 32 + c
                    blocks[block_idx, 0:2] = one
                    blocks[block_idx, 2:34] = expanded[row, k0:k1].view(np.uint8)
                    block_idx += 1

        return blocks.reshape(n, (k // 32) * 34)

    def _quantize_w8pc_a8pt_bytes(self, data_torch: Tensor) -> np.ndarray:
        data = data_torch.detach().cpu().to(torch.float32).numpy()
        if data.ndim != 2:
            raise ValueError("W8PC_A8PT conversion expects a 2D tensor")

        n, k = data.shape
        if n % 32 != 0 or k % 32 != 0:
            raise ValueError(f"W8PC_A8PT tensor shape must be 32-aligned, got {(n, k)}")

        scales = np.max(np.abs(data), axis=1).astype(np.float32) / np.float32(127.0)
        blocks = np.empty((n // 32 * k // 32 * 32, 34), dtype=np.uint8)
        ...
        for n_tile in range(n // 32):
            for k_tile in range(k // 32):
                ...
                for c in range(32):
                    row = n_tile * 32 + c
                    scale = scales[row]
                    blocks[block_idx, 0:2] = np.array([scale], dtype=np.float16).view(np.uint8)
                    if scale == 0:
                        q = np.zeros(32, dtype=np.int8)
                    else:
                        q = gguf.quants.np_roundf(data[row, k0:k1] / scale)
                        q = np.clip(q, -127, 127).astype(np.int8)
                    blocks[block_idx, 2:34] = q.view(np.uint8)
                    block_idx += 1

        return blocks.reshape(n, (k // 32) * 34)

    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor | np.ndarray]]:
        new_name = self.map_tensor_name(name)
        if self._is_htp_matmul_tensor(new_name, bid):
            ...
            if self._use_lpbq_a8w8_convert():
                return [(new_name, self._quantize_lpbq_a8w8_bytes(data_torch, name, new_name))]

            if self._use_w8pc_a8pt_convert():
                return [(new_name, self._quantize_w8pc_a8pt_bytes(data_torch))]
    ...
    def prepare_tensors(self):
        ...
        for name, data_torch in chain(self.generate_extra_tensors(), self.get_tensors()):
            ...
            for new_name, data_torch in (self.modify_tensors(data_torch, name, bid)):
                ...
                prequantized_qtype: gguf.GGMLQuantizationType | None = None
                if isinstance(data_torch, np.ndarray):
                    data = data_torch
                    if data.dtype == np.uint8:
                        prequantized_qtype = gguf.GGMLQuantizationType.Q8_0
                else:
                    data = data_torch.numpy()

                # if data ends up empty, it means data_torch was a scalar tensor -> restore
                if len(data.shape) == 0:
                    data = data_torch.numpy() if not isinstance(data_torch, np.ndarray) else data

                n_dims = len(data.shape)
                data_qtype: gguf.GGMLQuantizationType | bool = prequantized_qtype if prequantized_qtype is not None else self.tensor_force_quant(name, new_name, bid, n_dims)

                # Most of the codebase that takes in 1D tensors or norms only handles F32 tensors
                if prequantized_qtype is None and (n_dims <= 1 or new_name.endswith("_norm.weight")):
                    data_qtype = gguf.GGMLQuantizationType.F32

                # Conditions should closely match those in llama_model_quantize_internal in llama.cpp
                # Some tensor types are always in float32
                if prequantized_qtype is None and data_qtype is False and (
                    any(
                ...
                if prequantized_qtype is None and data_qtype is False and any(
                    self.match_model_tensor_name(new_name, key, bid)
                    for key in (
                ...
                if prequantized_qtype is None and isinstance(data_qtype, bool):
