...
ggml_backend_htp_context::ggml_backend_htp_context() : mapper(3 * 1024UL * 1024 * 1024, true) {
    ...
    const char * skip_env = getenv("SKIP_HTP_OPS");
    // pure_fp16 uses HMX-layout weights, so an accidental "SKIP_HTP_OPS=0"
    // must not disable HTP and send permuted tensors to the CPU fallback path.
    if (skip_env && (strcmp(skip_env, "1") == 0 || strcasecmp(skip_env, "true") == 0 ||
                     strcasecmp(skip_env, "yes") == 0 || strcasecmp(skip_env, "on") == 0)) {
        skip_htp_ops = true;
    }
}
