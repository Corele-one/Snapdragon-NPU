...
static std::string default_npu_mode() {
    const char * mode = std::getenv("LLAMA_NPU_MODE");
    return mode && mode[0] ? std::string(mode) : std::string("baseline");
}
...
struct slot_params {
    ...
    std::string npu_mode = default_npu_mode();
...
struct server_task {
    ...
    static slot_params params_from_json_cmpl(
        ...
        params.npu_mode          = json_value(data, "npu_mode", default_npu_mode());
...
struct server_context {
    ...
    bool launch_slot_with_task(server_slot & slot, const server_task & task) {
        ...
        if (slot.params.npu_mode != "baseline" && slot.params.npu_mode != "lut_exp" &&
            slot.params.npu_mode != "int8pc" && slot.params.npu_mode != "w8pc_a8pt" &&
            slot.params.npu_mode != "w8pc-a8pt" &&
            slot.params.npu_mode != "pure_fp16" && slot.params.npu_mode != "pure-fp16" &&
            slot.params.npu_mode != "fp16" &&
            slot.params.npu_mode != "lpbq_int8" && slot.params.npu_mode != "lpbq-int8" &&
            slot.params.npu_mode != "lpbq-w4a8-int8" && slot.params.npu_mode != "w4a8_lpbq") {
            SLT_WRN(slot, "unknown npu_mode '%s', falling back to baseline\n", slot.params.npu_mode.c_str());
            slot.params.npu_mode = "baseline";
        }
#if defined(_WIN32)
        _putenv_s("LLAMA_NPU_MODE", slot.params.npu_mode.c_str());
#else
        setenv("LLAMA_NPU_MODE", slot.params.npu_mode.c_str(), 1);
#endif
        SLT_INF(slot, "npu_mode = %s\n", slot.params.npu_mode.c_str());
