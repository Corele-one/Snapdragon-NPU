...
Options:
  ...
  --scna-variant VARIANT --workers auto|1..6 --scna-width 8
...
case "$scna_variant" in
  stage1_dynamic_row|prepare_once_row|pair_shared_dynamic|pair_static_d8|pair_d8_fma_noinline|pair_d8_fma_inline|optimized) ;;
  *) echo "Unsupported --scna-variant: $scna_variant" >&2; exit 2 ;;
esac
...
variant_artifact="$project_dir/artifacts/variants/$scna_variant/libhtp_ops_skel.so"
...
if [[ "$mode" == "ping" ]]; then
  ...
  adb shell "cd '$remote_dir' && LD_LIBRARY_PATH=. DSP_LIBRARY_PATH='./cdsp;./dsp;.' ./htp_ops_test --scna-exp-bench --scna-variant '$scna_variant' --scna-width '$scna_width' --warmup '$warmup' --iters '$iters'"
else
  event_arg="--no-events"
  [[ "$print_events" == "1" ]] && event_arg=""
  reference_arg=""
  [[ "$compare_reference" == "1" ]] && reference_arg="--compare-reference"
  adb shell "cd '$remote_dir' && LD_LIBRARY_PATH=. DSP_LIBRARY_PATH='./cdsp;./dsp;.' ./htp_ops_test --figure8-attn --mode '$mode' --scna-variant '$scna_variant' --workers '$workers' --scna-width '$scna_width' --mask-mode '$mask_mode' --qo-len '$qo_len' --kv-len '$kv_len' --n-heads '$n_heads' --n-kv-heads '$n_kv_heads' --head-dim '$head_dim' --warmup '$warmup' --iters '$iters' $event_arg $reference_arg"
fi
