...
HOT_SYMBOL = {
    "stage1_dynamic_row": ("stage1_dynamic_row", "hvx_scna_exp2_pair_vhf"),
    "prepare_once_row": ("prepared_dynamic_row", "hvx_scna_exp2_pair_vhf"),
    "pair_shared_dynamic": ("pair_shared_dynamic_qf16",),
    "pair_static_d8": ("pair_static_d8_qf16",),
...
def main():
    ...
    for variant in VARIANTS:
        ...
        selected = {name: body for name, body in blocks.items()
                    if any(token in name for token in HOT_SYMBOL[variant])}
        ...
        if variant in ("stage1_dynamic_row", "prepare_once_row"):
            row["effective_pair_splat"] = 2 * sum(metric(body)["splat"] for name, body in selected.items()
                                                   if "dynamic_row" in name)
        else:
