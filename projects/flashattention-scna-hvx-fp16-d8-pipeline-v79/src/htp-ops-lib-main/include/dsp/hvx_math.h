...
/* v79+ XQF range reduction from Qualcomm's qhmath HVX exp2 path.  The older
 * round-to-zero shortcut above has input-dependent errors on nonuniform
 * Attention scores, so correctness-gated paths use this implementation. */
static HVX_INLINE_ALWAYS HVX_Vector hvx_my_exp2_xqf_vhf(HVX_Vector input) {
  ...
  const HVX_Vector min_supported = Q6_Vh_vsplat_R(0xcb00);  // -14.0; smaller values are negligible here
