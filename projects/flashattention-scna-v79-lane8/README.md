...
这是 SCNA 8-lane 分组优化的独立实验项目，由 `flashattention-scna-v79-framework` 复制创建。项目已经实现 SCOPE direct d8 FP16 evaluator 的 `serial`/`lane8` runtime 切换、single/paired microbenchmark、Attention 热路径接入、FP32 reference gate、DSP qtimer 事件和 Perfetto 导出；原 framework 与 v81 demo 不会被此目录的构建或实验覆盖。
...
| `scna-fp16 --scna-layout serial` | SCNA direct d8 | 每个 HVX lane 独立计算一个 score，循环 8 个 neuron | SCNA 主因果对照。 |
| `scna-fp16 --scna-layout lane8` | SCNA direct d8 grouped | 每 8 lane 计算一个 score，并执行 `vlut16` 扩展、`vlalign` 归约与 `vdeal` 压缩 | 本轮待验证优化。 |
...
└── tools/
    ...
    ├── audit_perfetto_trace.py
    ├── combine_scna_perfetto_traces.py
    ├── analyze_scna_disassembly.py
    ├── analyze_scna_lane8_results.py
...
./scripts/deploy_and_smoke.sh --mode scna-fp16 --scna-layout serial --scna-width 8 --qo-len 4 --kv-len 4096
./scripts/deploy_and_smoke.sh --mode scna-fp16 --scna-layout lane8 --scna-width 8 --qo-len 4 --kv-len 4096
./scripts/deploy_and_smoke.sh --mode scna-exp-bench --scna-layout lane8 --iters 512000
```

成功的 attention 运行会输出 `FIG8_ATTENTION_*` 记录和 `ret=0`。使用 `--compare-reference` 时还会输出 `FIG8_ATTENTION_COMPARE`；只有其中 `pass=1` 才表示满足 RMSE/max-abs gate。
...
```bash
./scripts/run_scna_lane8_experiment.sh
```

脚本会创建带时间戳的 `results/v79/scna-lane8/` 子目录，采集三 session headline、microbenchmark、正确性矩阵、静态反汇编以及独立 Perfetto replay。headline 使用 `--no-events`，trace replay 才启用软件事件，避免将记录开销混入主性能数字。d16/d32 只有在 d8 正确性、paired microkernel 和四个 Qo 的 95% CI gate 全部通过时才应扩展。
