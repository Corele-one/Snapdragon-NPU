...
Options:
  --mode ping|baseline|lut-exp  Test to run after deployment (default: ping)
  --dsp-arch v73|v79|v81        DSP build target (default: v79)
...
if [[ "$mode" == "ping" ]]; then
  ...
  adb shell "cd '$remote_dir' && LD_LIBRARY_PATH=. DSP_LIBRARY_PATH='./cdsp;./dsp;.' ./htp_ops_test --figure8-attn --mode '$mode' --qo-len '$qo_len' --kv-len '$kv_len' --n-heads '$n_heads' --n-kv-heads '$n_kv_heads' --head-dim '$head_dim' --warmup '$warmup' --iters '$iters' --no-events"
fi
