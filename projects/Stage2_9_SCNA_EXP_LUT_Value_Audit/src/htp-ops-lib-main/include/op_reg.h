...
enum HtpOpsIndex {
  ...
  HTP_OPS_NONLINEAR_VALUE_AUDIT,
  HTP_OPS_MATMUL_VALUE_AUDIT,
  HTP_OPS_COUNT,
};

/* Stage 2.9 experiment-only ABI.  These values deliberately live after all
 * frozen Stage 2.75 op ids so the production ABI is not renumbered. */
enum NonlinearAuditEvaluator {
  NONLINEAR_AUDIT_EVALUATOR_LUT_EXP = 1,
  NONLINEAR_AUDIT_EVALUATOR_SCNA_FP16 = 2,
};

enum NonlinearAuditDistribution {
  NONLINEAR_AUDIT_DISTRIBUTION_EXHAUSTIVE_FP16 = 1,
  NONLINEAR_AUDIT_DISTRIBUTION_DENSE = 2,
  NONLINEAR_AUDIT_DISTRIBUTION_ATTENTION = 3,
  NONLINEAR_AUDIT_DISTRIBUTION_BOUNDARY = 4,
};

enum AuditBuildFlavor {
  AUDIT_BUILD_FAIR_COMBINED = 0,
  AUDIT_BUILD_LUT_ONLY = 1,
  AUDIT_BUILD_SCNA_ONLY = 2,
};
...
enum LlmNpuModeFlags {
  ...
  /* Query-task rows are encoded as log2(rows / 4): 0,1,2,3 -> 4,8,16,32.
   * This keeps the RPC ABI stable while allowing the Stage 2.75 scheduler
   * ablation to change only task granularity. */
  LLM_NPU_MODE_Q_TASK_ROWS_SHIFT = 20,
  LLM_NPU_MODE_Q_TASK_ROWS_MASK  = 3 << LLM_NPU_MODE_Q_TASK_ROWS_SHIFT,
  LLM_NPU_MODE_Q_TASK_ROWS_AUTO  = 1 << 22,
};
...
/* Compile-time experiment implementations.  They all retain the public
 * SCNA_VARIANT_PAIR_STATIC_D8 runtime encoding and are shipped as separately
 * hashed DSP artifacts. */
#define SCNA_KERNEL_IMPL_STATIC_D8_REF 0
#define SCNA_KERNEL_IMPL_D7_SERIAL 1
#define SCNA_KERNEL_IMPL_D7_SCALAR_W 2
#define SCNA_KERNEL_IMPL_D7_PAIRRET_NOINLINE 3
#define SCNA_KERNEL_IMPL_D7_PAIRRET_INLINE 4
#define SCNA_KERNEL_IMPL_D7_QUAD_PIPELINE 5
#define SCNA_KERNEL_IMPL_D7_PREBROADCAST 6
#define SCNA_KERNEL_IMPL_QF16_TREE_CONTROL 7
#define SCNA_KERNEL_IMPL_PIECEWISE_CONTROL 8
#define SCNA_KERNEL_IMPL_COMBINED_CONFIRM 9
#define SCNA_KERNEL_IMPL_D7_PAIRRET_TWO_ACC 10
#define SCNA_KERNEL_IMPL_D7_PAIRRET_SHORT_CONST 11
#define SCNA_KERNEL_IMPL_D7_PAIRRET_TWO_NEURON 12
#define SCNA_KERNEL_IMPL_D7_PAIRRET_SCALE_PROBE 13
#define SCNA_KERNEL_IMPL_COUNT 14
...
/* Stage 2.9.1 diagnostic ABI.  It is deliberately a separate op so model
 * execution cannot silently enter an oracle or diagnostic path. */
enum MatMulAuditDType {
  MATMUL_AUDIT_DTYPE_F16 = 1,
  MATMUL_AUDIT_DTYPE_IQ4_NL = 2,
  MATMUL_AUDIT_DTYPE_Q8_0 = 3,
};

enum MatMulAuditPath {
  MATMUL_AUDIT_PATH_PRODUCTION = 0,
  MATMUL_AUDIT_PATH_SEQUENTIAL = 1,
  MATMUL_AUDIT_PATH_PIPELINE = 2,
  MATMUL_AUDIT_PATH_OUTPUT_STATIONARY = 3,
};

struct MatMulValueAuditParams {
  struct RpcmemBufAddr output;
  struct RpcmemBufAddr activation;
  struct RpcmemBufAddr weight;
  struct RpcmemBufAddr result;
  int32_t m;
  int32_t k;
  int32_t n;
  int32_t dtype;
  int32_t requested_path;
  int32_t seed;
  int64_t weight_bytes;
} __attribute__((packed));

struct MatMulValueAuditResult {
...
struct ScnaExp2BenchResult {
  ...
  int32_t scale_head_dim;
  uint32_t scale_fp16_bits;
  int64_t scale_only_elapsed_us;
  int64_t separate_scale_lut_elapsed_us;
  int64_t separate_scale_scna_elapsed_us;
  int64_t fused_scale_scna_elapsed_us;
  float fused_scale_max_abs_diff;
  int32_t fused_scale_mismatches;
  int32_t fused_scale_samples;
  uint32_t scale_checksum_bits;
} __attribute__((packed));

struct NonlinearValueAuditParams {
...
struct NonlinearValueAuditResult {
  int32_t schema_version;
  int32_t evaluator;
  int32_t build_flavor;
  int32_t distribution;
  int32_t elements;
  int32_t iters;
  int32_t nonfinite_count;
  int32_t negative_count;
  int32_t monotonic_violations;
  int32_t reserved0;
  int64_t elapsed_ticks;
  ...
  int64_t vtcm_total_bytes;
  int64_t vtcm_reserved_bytes;
  int64_t vtcm_seq_capacity_bytes;
  int64_t lut_table_bytes;
  int64_t lut_init_us;
} __attribute__((packed));
...
struct Figure8ProfileRecord {
  ...
  int64_t o_store;
  int64_t scna_exp;
  int64_t state_update;
  int64_t rowmax_mem;
  int64_t rowsum_reduce;
  int64_t rowmax_mem_ticks;
  int64_t rowsum_reduce_ticks;
  int64_t state_update_ticks;
