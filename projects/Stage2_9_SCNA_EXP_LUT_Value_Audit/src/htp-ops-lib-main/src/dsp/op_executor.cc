...
const char *dsp_op_name(uint32_t op) {
  switch (op) {
    ...
    case HTP_OPS_NONLINEAR_VALUE_AUDIT:
      return "nonlinear_value_audit";
    case HTP_OPS_MATMUL_VALUE_AUDIT:
      return "matmul_value_audit";
    default:
      return "unknown";
  }
}
...
#ifndef AUDIT_BUILD_FLAVOR
#define AUDIT_BUILD_FLAVOR AUDIT_BUILD_FAIR_COMBINED
#endif

static bool audit_evaluator_supported(int evaluator) {
  if (evaluator == NONLINEAR_AUDIT_EVALUATOR_LUT_EXP) {
    return AUDIT_BUILD_FLAVOR != AUDIT_BUILD_SCNA_ONLY;
  }
  if (evaluator == NONLINEAR_AUDIT_EVALUATOR_SCNA_FP16) {
    return AUDIT_BUILD_FLAVOR != AUDIT_BUILD_LUT_ONLY;
  }
  return false;
}

static bool audit_mode_supported(int mode_flags) {
  const bool wants_lut = (mode_flags & LLM_NPU_MODE_LUT_EXP) != 0;
  const bool wants_scna = (mode_flags & LLM_NPU_MODE_SCNA_FP16) != 0;
  if (wants_lut && wants_scna) return false;
  if (wants_lut && AUDIT_BUILD_FLAVOR == AUDIT_BUILD_SCNA_ONLY) return false;
  if (wants_scna && AUDIT_BUILD_FLAVOR == AUDIT_BUILD_LUT_ONLY) return false;
  return true;
}

static int nonlinear_value_audit_run(const __fp16 *input, __fp16 *output, NonlinearValueAuditResult *result,
                                     int elements, int evaluator, int distribution, int warmup, int iters) {
  if (!input || !output || !result || elements <= 0 || elements > 32768 || (elements % 128) != 0 ||
      warmup < 0 || iters <= 0 || !audit_evaluator_supported(evaluator)) {
    return -1;
  }

  uint8_t *vtcm = reinterpret_cast<uint8_t *>(vtcm_manager_get_vtcm_base());
  const size_t bytes = static_cast<size_t>(elements) * sizeof(__fp16);
  const size_t output_offset = (bytes + VLEN - 1) / VLEN * VLEN;
  if (!vtcm || output_offset + bytes > vtcm_manager_get_seq_capacity()) return -2;
  __fp16 *stage_input = reinterpret_cast<__fp16 *>(vtcm);
  __fp16 *stage_output = reinterpret_cast<__fp16 *>(vtcm + output_offset);
  std::memcpy(stage_input, input, bytes);

  const __fp16 *table = reinterpret_cast<const __fp16 *>(
      vtcm_manager_query_area("safe_softmax::exp2_hf_qf16"));
  if (evaluator == NONLINEAR_AUDIT_EVALUATOR_LUT_EXP && !table) return -3;

  scna_exp2_hvx_params_t scna_params = {};
  if (evaluator == NONLINEAR_AUDIT_EVALUATOR_SCNA_FP16) {
    const int mode_flags = LLM_NPU_MODE_SCNA_FP16 | LLM_NPU_MODE_SCNA_D8 |
                           (SCNA_VARIANT_PAIR_STATIC_D8 << 10);
    if (scna_exp2_prepare_hvx_params(&scna_params, mode_flags) != 0) return -4;
  }

  auto run_once = [&]() {
    if (evaluator == NONLINEAR_AUDIT_EVALUATOR_LUT_EXP) {
      for (int i = 0; i < elements; i += 128) {
        const HVX_Vector in0 = vmem(stage_input + i);
        const HVX_Vector in1 = vmem(stage_input + i + 64);
        const HVX_Vector off0 = Q6_Vh_vasl_VhR(in0, 1);
        const HVX_Vector off1 = Q6_Vh_vasl_VhR(in1, 1);
        Q6_vgather_ARMVh(reinterpret_cast<HVX_Vector *>(stage_output + i), (size_t) table, 65535, off0);
        Q6_vgather_ARMVh(reinterpret_cast<HVX_Vector *>(stage_output + i + 64), (size_t) table, 65535, off1);
      }
      volatile HVX_Vector *sync = reinterpret_cast<volatile HVX_Vector *>(stage_output + elements - 64);
      const HVX_Vector sink = *sync;
      (void) sink;
    } else {
      for (int i = 0; i < elements; i += 128) {
        const HVX_VectorPair pair = hvx_scna_exp2_pair_hot_return_vhf(
            vmem(stage_input + i), vmem(stage_input + i + 64), &scna_params);
        vmem(stage_output + i) = Q6_V_lo_W(pair);
        vmem(stage_output + i + 64) = Q6_V_hi_W(pair);
      }
    }
    __asm__ volatile("" : : "m"(*stage_output) : "memory");
  };

  for (int i = 0; i < warmup; ++i) run_once();
  const int64_t t0 = HAP_perf_get_qtimer_count();
  for (int i = 0; i < iters; ++i) run_once();
  const int64_t elapsed_ticks = HAP_perf_get_qtimer_count() - t0;
  std::memcpy(output, stage_output, bytes);
  ...
  for (int i = 0; i < elements; ++i) {
    const float x = static_cast<float>(stage_input[i]);
    const float actual = static_cast<float>(stage_output[i]);
    const float reference = exp2f(x);
    if (!std::isfinite(actual)) ++nonfinite;
    if (actual < 0.0f) ++negative;
    if (distribution != NONLINEAR_AUDIT_DISTRIBUTION_ATTENTION && i > 0 && actual < previous) ++monotonic;
    previous = actual;
    if (std::isfinite(actual)) {
      const float error = actual - reference;
      const float abs_error = std::fabs(error);
      error_sq += static_cast<double>(error) * error;
      reference_sq += static_cast<double>(reference) * reference;
      abs_sum += abs_error;
      if (abs_error > max_abs) max_abs = abs_error;
    }
    uint16_t bits;
    std::memcpy(&bits, &stage_output[i], sizeof(bits));
    checksum ^= bits;
    checksum *= UINT64_C(1099511628211);
  }

  const int64_t total_vtcm = static_cast<int64_t>(vtcm_manager_get_total_size());
  const int64_t seq_vtcm = static_cast<int64_t>(vtcm_manager_get_seq_capacity());
  *result = NonlinearValueAuditResult {
    .schema_version = 1,
    .evaluator = evaluator,
    .build_flavor = AUDIT_BUILD_FLAVOR,
    ...
    .elapsed_us = static_cast<int64_t>(HAP_perf_qtimer_count_to_us(elapsed_ticks)),
    .checksum = checksum,
    .rmse = static_cast<float>(std::sqrt(error_sq / elements)),
    .max_abs_error = max_abs,
    .relative_l2 = reference_sq > 0.0 ? static_cast<float>(std::sqrt(error_sq / reference_sq)) : 0.0f,
    .mean_abs_error = static_cast<float>(abs_sum / elements),
    .vtcm_total_bytes = total_vtcm,
    .vtcm_reserved_bytes = total_vtcm - seq_vtcm,
    .vtcm_seq_capacity_bytes = seq_vtcm,
    .lut_table_bytes = audit_lut_table_bytes(),
    .lut_init_us = audit_lut_init_us(),
  };
  return 0;
}
...
int execute_op_simple(struct OpComputeRequest *req) {
  ...
  int ret = 0;
  auto checksum64 = [](const void *data, size_t bytes) -> uint64_t {
  ...
  switch (req->op) {
    ...
    case HTP_OPS_SCNA_EXP2_BENCH:
      {
        ...
        if (params->width != 8 ||
            ...
            params->variant < 0 || params->variant >= SCNA_VARIANT_COUNT || params->warmup < 0 || params->iters <= 0) {
          ret = -1;
          break;
        }
    ...
    case HTP_OPS_NONLINEAR_VALUE_AUDIT:
      {
        auto params = reinterpret_cast<NonlinearValueAuditParams *>(req->payload);
        if (params->elements <= 0 || params->elements > 32768 || (params->elements % 128) != 0 ||
            params->warmup < 0 || params->iters <= 0 || !audit_evaluator_supported(params->evaluator)) {
          ret = -1;
          break;
        }
        const size_t data_size = static_cast<size_t>(params->elements) * sizeof(__fp16);
        add_buffer(in_bufs, params->input, data_size);
        add_buffer(out_bufs, params->output, data_size);
        add_buffer(out_bufs, params->result, sizeof(NonlinearValueAuditResult));
        validate_in_bufs();
        ret = nonlinear_value_audit_run(reinterpret_cast<const __fp16 *>(IN_PTR(0)),
                                        reinterpret_cast<__fp16 *>(OUT_PTR(0)),
                                        reinterpret_cast<NonlinearValueAuditResult *>(OUT_PTR(1)),
                                        params->elements, params->evaluator, params->distribution,
                                        params->warmup, params->iters);
        validate_out_bufs();
      }
      break;
    ...
    case HTP_OPS_MATMUL_VALUE_AUDIT:
      {
        auto params = reinterpret_cast<MatMulValueAuditParams *>(req->payload);
        if (params->m <= 0 || params->k <= 0 || params->n <= 0 || params->k % 32 || params->n % 32 ||
            params->weight_bytes <= 0 || params->dtype < MATMUL_AUDIT_DTYPE_F16 ||
            params->dtype > MATMUL_AUDIT_DTYPE_Q8_0 || params->requested_path < MATMUL_AUDIT_PATH_PRODUCTION ||
            params->requested_path > MATMUL_AUDIT_PATH_OUTPUT_STATIONARY) {
          ret = -1;
          break;
        }
        const size_t output_size = (size_t) params->m * params->n * sizeof(float);
        const size_t activation_size = (size_t) params->m * params->k * sizeof(float);
        const size_t weight_size = (size_t) params->weight_bytes;
        add_buffer(out_bufs, params->output, output_size);
        add_buffer(in_bufs, params->activation, activation_size);
        add_buffer(in_bufs, params->weight, weight_size, params->dtype == MATMUL_AUDIT_DTYPE_F16);
        add_buffer(out_bufs, params->result, sizeof(MatMulValueAuditResult));
        validate_in_bufs();
        auto *result = reinterpret_cast<MatMulValueAuditResult *>(OUT_PTR(1));
        memset(result, 0, sizeof(*result));
        result->schema_version = 1;
        result->dtype = params->dtype;
        result->requested_path = params->requested_path;
        result->m = params->m;
        result->k = params->k;
        result->n = params->n;
        result->seed = params->seed;
        result->activation_checksum = checksum64(IN_PTR(0), activation_size);
        result->weight_checksum = checksum64(IN_PTR(1), weight_size);

        int actual_path = MATMUL_AUDIT_PATH_SEQUENTIAL;
        const int64_t t0 = trace_now_us();
        if (params->dtype == MATMUL_AUDIT_DTYPE_F16) {
          if (params->requested_path == MATMUL_AUDIT_PATH_PIPELINE ||
              params->requested_path == MATMUL_AUDIT_PATH_OUTPUT_STATIONARY) {
            ret = -2;
          } else {
            ret = hmx_mat_mul_permuted_w16a32(reinterpret_cast<float *>(OUT_PTR(0)),
                                              reinterpret_cast<float *>(IN_PTR(0)),
                                              reinterpret_cast<__fp16 *>(IN_PTR(1)), params->m,
                                              params->k, params->n, 0, 0, req->op, nullptr);
          }
        } else {
          const enum ggml_type type = params->dtype == MATMUL_AUDIT_DTYPE_IQ4_NL
            ? GGML_TYPE_IQ4_NL : GGML_TYPE_Q8_0;
          if (params->requested_path == MATMUL_AUDIT_PATH_OUTPUT_STATIONARY) {
            actual_path = MATMUL_AUDIT_PATH_OUTPUT_STATIONARY;
            ret = mat_mul_qk_0_d16a32_out_stationary(reinterpret_cast<float *>(OUT_PTR(0)),
                  reinterpret_cast<float *>(IN_PTR(0)), reinterpret_cast<uint8_t *>(IN_PTR(1)),
                  params->m, params->k, params->n, type, 0, 0, req->op, nullptr);
          } else {
            actual_path = (params->m >= 128 && params->k <= params->n)
              ? MATMUL_AUDIT_PATH_PIPELINE : MATMUL_AUDIT_PATH_SEQUENTIAL;
            if (params->requested_path != MATMUL_AUDIT_PATH_PRODUCTION &&
                params->requested_path != actual_path) {
              ret = -3;
            } else {
              ret = hmx_mat_mul_permuted_qk_0_d16a32(reinterpret_cast<float *>(OUT_PTR(0)),
                    reinterpret_cast<float *>(IN_PTR(0)), reinterpret_cast<uint8_t *>(IN_PTR(1)),
                    params->m, params->k, params->n, type, 0, 0, req->op, nullptr);
            }
          }
        }
        result->elapsed_us = trace_now_us() - t0;
        result->actual_path = actual_path;
        result->status = ret;
        result->output_checksum = ret == 0 ? checksum64(OUT_PTR(0), output_size) : 0;
        validate_out_bufs();
      }
      break;
    ...
    case HTP_OPS_FLASH_ATTN_QO_F32_KV_F16:
      {
        ...
        if (!audit_mode_supported(params->mode_flags)) {
          FARF(ALWAYS, "Stage2.9 artifact flavor %d rejects mode_flags=0x%x", AUDIT_BUILD_FLAVOR,
               params->mode_flags);
          ret = -9;
          break;
        }
    ...
    case HTP_OPS_FLASH_ATTN_PROFILE_QO_F32_KV_F16:
      {
        ...
        if (!audit_mode_supported(params->attn.mode_flags)) {
          FARF(ALWAYS, "Stage2.9 artifact flavor %d rejects mode_flags=0x%x", AUDIT_BUILD_FLAVOR,
               params->attn.mode_flags);
          ret = -9;
          break;
        }
        ...
        profile->event_count  = 0;
        profile->event_overflow = 0;
        profile->active_workers = 0;
        profile->hvx_contexts = 0;
        profile->vtcm_worker_cap = 0;
        profile->tasks        = 0;
        profile->q_task_rows  = 0;
        profile->schema_version = 2;
        profile->vtcm_total_bytes = static_cast<int64_t>(vtcm_manager_get_total_size());
        profile->vtcm_seq_capacity_bytes = static_cast<int64_t>(vtcm_manager_get_seq_capacity());
        profile->vtcm_reserved_bytes = profile->vtcm_total_bytes - profile->vtcm_seq_capacity_bytes;
        profile->lut_table_bytes = audit_lut_table_bytes();
        profile->lut_init_us = audit_lut_init_us();
