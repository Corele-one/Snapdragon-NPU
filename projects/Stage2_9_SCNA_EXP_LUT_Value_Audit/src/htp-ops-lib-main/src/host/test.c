...
struct Figure8AttnConfig {
  ...
  int         scna_exp_bench;
  int         value_audit;
  int         value_audit_elements;
  int         matmul_audit;
  const char *matmul_dtype;
  const char *matmul_path;
  int         matmul_m;
  int         matmul_k;
  int         matmul_n;
  int         workers;
...
static int matmul_audit_dtype_id(const char *name);
static int matmul_audit_path_id(const char *name);

static void figure8_print_usage(const char *prog) {
  fprintf(stderr,
          "Usage: %s --figure8-attn [--mode baseline|lut-exp|scna-fp16] [--scna-variant VARIANT] [--workers auto|1..6]\n"
          "          VARIANT: stage1_dynamic_row|prepare_once_row|pair_shared_dynamic|pair_static_d8\n"
          "                   |pair_d8_fma_noinline|pair_d8_fma_inline|optimized\n"
          "          [--scna-width 8|16|32] [--mask-mode full|causal|padding] [--qo-len N] [--kv-len N]\n"
          "          [--q-task-rows auto|4|8|16|32] [--n-heads N] [--n-kv-heads N] [--head-dim N] [--warmup N] [--iters N]\n"
          ...
          "       %s --value-audit --mode lut-exp|scna-fp16 --distribution exhaustive-fp16|dense|attention|boundary\n"
          "          [--elements N] [--warmup N] [--iters N]\n"
          "       %s --matmul-audit --dtype f16|iq4_nl|q8_0 --execution-path production|sequential|pipeline|output_stationary\n"
          "          --m N --k N --n N [--seed UINT32]\n",
          prog,
          prog,
          prog,
          prog,
...
static int parse_figure8_args(int argc, char **argv, struct Figure8AttnConfig *cfg) {
  *cfg = (struct Figure8AttnConfig) {
    ...
    .scna_exp_bench = 0,
    .value_audit = 0,
    .value_audit_elements = 4096,
    .matmul_audit = 0,
    .matmul_dtype = "f16",
    .matmul_path = "production",
    .matmul_m = 1,
    .matmul_k = 32,
    .matmul_n = 32,
    .workers = 1,
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (strcmp(arg, "--figure8-attn") == 0) {
    ...
    } else if (strcmp(arg, "--value-audit") == 0) {
      cfg->value_audit = 1;
    } else if (strcmp(arg, "--matmul-audit") == 0) {
      cfg->matmul_audit = 1;
    ...
    } else if (strcmp(arg, "--scna-variant") == 0) {
      if (++i >= argc) {
        fprintf(stderr, "Missing value for --scna-variant\n");
        return -1;
      }
      cfg->scna_variant = argv[i];
    ...
    } else if (strcmp(arg, "--elements") == 0) {
      if (++i >= argc || parse_int_cli_value(arg, argv[i], &cfg->value_audit_elements)) return -1;
    } else if (strcmp(arg, "--dtype") == 0) {
      if (++i >= argc) return -1;
      cfg->matmul_dtype = argv[i];
    } else if (strcmp(arg, "--execution-path") == 0) {
      if (++i >= argc) return -1;
      cfg->matmul_path = argv[i];
      ...
      if (++i >= argc) return -1;
      if (strcmp(argv[i], "auto") == 0) cfg->workers = 0;
      else if (parse_int_cli_value(arg, argv[i], &cfg->workers)) return -1;
    } else if (strcmp(arg, "--q-task-rows") == 0) {
      if (++i >= argc) return -1;
      if (strcmp(argv[i], "auto") == 0) cfg->q_task_rows = 0;
      else if (parse_int_cli_value(arg, argv[i], &cfg->q_task_rows)) return -1;
  ...
  if (!cfg->enabled && !cfg->roofline_fp16_bench && !cfg->roofline_hmx_fp16_bench &&
      ...
      !cfg->roofline_signed_int8_zero_overhead_bench && !cfg->scna_exp_bench && !cfg->value_audit &&
      !cfg->matmul_audit) {
    return 0;
  }
  if (!cfg->enabled && !cfg->scna_exp_bench && !cfg->value_audit && !cfg->matmul_audit) {
    return 0;
  }
  ...
  if (cfg->value_audit && strcmp(cfg->lut_distribution, "exhaustive-fp16") != 0 &&
      strcmp(cfg->lut_distribution, "dense") != 0 && strcmp(cfg->lut_distribution, "attention") != 0 &&
      strcmp(cfg->lut_distribution, "boundary") != 0) {
    fprintf(stderr, "Unsupported value-audit --distribution: %s\n", cfg->lut_distribution);
    return -1;
  }
  if (cfg->value_audit && strcmp(cfg->mode, "lut-exp") != 0 && strcmp(cfg->mode, "scna-fp16") != 0) {
    fprintf(stderr, "--value-audit requires --mode lut-exp or scna-fp16\n");
    return -1;
  }
  if (cfg->value_audit && strcmp(cfg->lut_distribution, "exhaustive-fp16") != 0 &&
      (cfg->value_audit_elements > 32768 || (cfg->value_audit_elements % 128) != 0)) {
    fprintf(stderr, "--elements must be a multiple of 128 and <=32768\n");
    return -1;
  }
  if (cfg->matmul_audit && (matmul_audit_dtype_id(cfg->matmul_dtype) < 0 ||
      matmul_audit_path_id(cfg->matmul_path) < 0 || cfg->matmul_k % 32 || cfg->matmul_n % 32)) {
    fprintf(stderr, "invalid --matmul-audit dtype/path/shape\n");
    return -1;
  }
  ...
  if (strcmp(cfg->scna_layout, "serial") != 0) {
    fprintf(stderr, "This FP16 D8 optimization project supports only --scna-layout serial\n");
    return -1;
  }
  if (strcmp(cfg->scna_variant, "stage1_dynamic_row") != 0 &&
      strcmp(cfg->scna_variant, "prepare_once_row") != 0 &&
      strcmp(cfg->scna_variant, "pair_shared_dynamic") != 0 &&
      strcmp(cfg->scna_variant, "pair_static_d8") != 0 &&
      strcmp(cfg->scna_variant, "pair_d8_fma_noinline") != 0 &&
      strcmp(cfg->scna_variant, "pair_d8_fma_inline") != 0 &&
      strcmp(cfg->scna_variant, "optimized") != 0) {
    fprintf(stderr, "Unsupported --scna-variant: %s\n", cfg->scna_variant);
    return -1;
  }
  if (cfg->workers < 0 || cfg->workers > 6) {
    fprintf(stderr, "--workers must be auto or in [1,6]\n");
    return -1;
  }
  if (cfg->q_task_rows != 0 && cfg->q_task_rows != 4 && cfg->q_task_rows != 8 &&
      cfg->q_task_rows != 16 && cfg->q_task_rows != 32) {
    fprintf(stderr, "--q-task-rows must be auto or one of 4, 8, 16, or 32\n");
    return -1;
  }
  ...
  if (cfg->compare_scna_layout) {
    fprintf(stderr, "--compare-scna-layout is unavailable in the serial-only D8 optimization project\n");
    return -1;
  }
  return 0;
}

static int scna_variant_id(const char *variant) {
  if (strcmp(variant, "prepare_once_row") == 0) return SCNA_VARIANT_PREPARE_ONCE_ROW;
  if (strcmp(variant, "pair_shared_dynamic") == 0) return SCNA_VARIANT_PAIR_SHARED_DYNAMIC;
  if (strcmp(variant, "pair_static_d8") == 0) return SCNA_VARIANT_PAIR_STATIC_D8;
  if (strcmp(variant, "pair_d8_fma_noinline") == 0) return SCNA_VARIANT_PAIR_D8_FMA_NOINLINE;
  if (strcmp(variant, "pair_d8_fma_inline") == 0) return SCNA_VARIANT_PAIR_D8_FMA_INLINE;
  if (strcmp(variant, "optimized") == 0) return SCNA_VARIANT_OPTIMIZED;
  return SCNA_VARIANT_STAGE1_DYNAMIC_ROW;
}
...
static int value_audit_send_request(struct MessageHeader *msg, size_t max_msg_size,
                                    const struct NonlinearValueAuditParams *params) {
  struct RequestHeader req_hdr = { .state = 0, .type = REQUEST_TYPE_OP_COMPUTE };
  struct OpComputeRequest compute_req = { .op = HTP_OPS_NONLINEAR_VALUE_AUDIT };
  msg->state.d = 0;
  msg->n_reqs = 1;
  msg->req_offsets[0] = message_header_size(msg);
  msg->req_offsets[1] = msg->req_offsets[0] + sizeof(req_hdr) + sizeof(compute_req) + sizeof(*params);
  if ((size_t) msg->req_offsets[1] > max_msg_size) return -1;
  uint8_t *p = (uint8_t *) message_header_get_request_ptr(msg, 0);
  *(struct RequestHeader *) p = req_hdr;
  p += sizeof(req_hdr);
  *(struct OpComputeRequest *) p = compute_req;
  p += sizeof(compute_req);
  *(struct NonlinearValueAuditParams *) p = *params;
  msg->state.v[0] = 1;
  if (figure8_wait_channel(msg, 30000000)) return -1;
  return message_header_get_request_ptr(msg, 0)->state;
}

static int matmul_audit_send_request(struct MessageHeader *msg, size_t max_msg_size,
                                     const struct MatMulValueAuditParams *params) {
  struct RequestHeader req_hdr = { .state = 0, .type = REQUEST_TYPE_OP_COMPUTE };
  struct OpComputeRequest compute_req = { .op = HTP_OPS_MATMUL_VALUE_AUDIT };
  msg->state.d = 0;
  msg->n_reqs = 1;
  msg->req_offsets[0] = message_header_size(msg);
  msg->req_offsets[1] = msg->req_offsets[0] + sizeof(req_hdr) + sizeof(compute_req) + sizeof(*params);
  ...
  *(struct OpComputeRequest *) p = compute_req;
  p += sizeof(compute_req);
  *(struct MatMulValueAuditParams *) p = *params;
  msg->state.v[0] = 1;
  if (figure8_wait_channel(msg, 120000000)) return -1;
  return message_header_get_request_ptr(msg, 0)->state;
}

static uint64_t audit_checksum64(const void *data, size_t bytes) {
...
static size_t audit_permuted_index(int ki, int nj, int k) {
  const int i0 = ki / 32, i1 = ki % 32;
  const int j0 = nj / 32, j1 = nj % 32;
  return ((size_t) j0 * (k / 32) + i0) * 1024 + (i1 & ~1) * 32 + j1 * 2 + (i1 & 1);
}

static int matmul_audit_dtype_id(const char *name) {
  if (strcmp(name, "f16") == 0) return MATMUL_AUDIT_DTYPE_F16;
  if (strcmp(name, "iq4_nl") == 0) return MATMUL_AUDIT_DTYPE_IQ4_NL;
  if (strcmp(name, "q8_0") == 0) return MATMUL_AUDIT_DTYPE_Q8_0;
  return -1;
}

static int matmul_audit_path_id(const char *name) {
  if (strcmp(name, "production") == 0) return MATMUL_AUDIT_PATH_PRODUCTION;
  if (strcmp(name, "sequential") == 0) return MATMUL_AUDIT_PATH_SEQUENTIAL;
  if (strcmp(name, "pipeline") == 0) return MATMUL_AUDIT_PATH_PIPELINE;
  if (strcmp(name, "output_stationary") == 0) return MATMUL_AUDIT_PATH_OUTPUT_STATIONARY;
  return -1;
}

static int run_matmul_value_audit(const struct Figure8AttnConfig *cfg) {
  static const float iq4_values[16] = {
    -127, -104, -83, -65, -49, -35, -22, -10, 1, 13, 25, 38, 53, 69, 89, 113,
  };
  const int m = cfg->matmul_m, k = cfg->matmul_k, n = cfg->matmul_n;
  const int dtype = matmul_audit_dtype_id(cfg->matmul_dtype);
  const int path = matmul_audit_path_id(cfg->matmul_path);
  const size_t activation_bytes = (size_t) m * k * sizeof(float);
  const size_t output_bytes = (size_t) m * n * sizeof(float);
  const size_t elements = (size_t) k * n;
  const size_t weight_bytes = dtype == MATMUL_AUDIT_DTYPE_F16 ? elements * sizeof(__fp16) :
    elements / 256 * (dtype == MATMUL_AUDIT_DTYPE_Q8_0 ? 272 : 144);
  const size_t max_msg_size = 4096;
  float *activation = NULL, *output = NULL;
  uint8_t *weight = NULL;
  struct MatMulValueAuditResult *result = NULL;
  void *chan = NULL;
  float *logical_weight = NULL, *reference = NULL, *permuted_values = NULL;
  int activation_fd = -1, output_fd = -1, weight_fd = -1, result_fd = -1, chan_fd = -1;
  int ret = 1;
  if (dtype < 0 || path < 0 || k % 32 || n % 32 || (dtype != MATMUL_AUDIT_DTYPE_F16 && k % 256)) {
    fprintf(stderr, "invalid matmul audit configuration\n");
    return 2;
  }
  logical_weight = (float *) malloc(elements * sizeof(float));
  permuted_values = (float *) malloc(elements * sizeof(float));
  reference = (float *) malloc(output_bytes);
  if (!logical_weight || !permuted_values || !reference ||
      alloc_shared_mem_buf((void **) &activation, &activation_fd, align_up(activation_bytes, 128)) ||
      alloc_shared_mem_buf((void **) &output, &output_fd, align_up(output_bytes, 128)) ||
      alloc_shared_mem_buf((void **) &weight, &weight_fd, align_up(weight_bytes, 128)) ||
      alloc_shared_mem_buf((void **) &result, &result_fd, align_up(sizeof(*result), 128)) ||
      alloc_shared_mem_buf(&chan, &chan_fd, max_msg_size)) goto end;
  memset(activation, 0, activation_bytes);
  ...
  for (int r = 0; r < m; ++r) {
    for (int z = 0; z < 16; ++z) {
      const int col = (int) (((uint64_t) cfg->seed * 131 + r * 977 + z * 1009) % (uint32_t) k);
      activation[(size_t) r * k + col] += (float) ((z % 9) - 4) / 16.0f;
    }
  }
  if (dtype == MATMUL_AUDIT_DTYPE_F16) {
    __fp16 *half_weight = (__fp16 *) weight;
    for (size_t p = 0; p < elements; ++p) {
      const int value = (int) ((p * 17 + cfg->seed * 13) % 255) - 127;
      permuted_values[p] = (float) (__fp16) ((float) value / 512.0f);
      half_weight[p] = (__fp16) permuted_values[p];
    }
  } else if (dtype == MATMUL_AUDIT_DTYPE_Q8_0) {
    const size_t super_blocks = elements / 256;
    for (size_t sb = 0; sb < super_blocks; ++sb) {
      __fp16 *scales = (__fp16 *) (weight + sb * 272);
      int8_t *quants = (int8_t *) (weight + sb * 272 + 16);
      for (int group = 0; group < 8; ++group) scales[group] = (__fp16) (1.0f / 128.0f);
      for (int x = 0; x < 256; ++x) {
        int q = (int) ((sb * 29 + x * 11 + cfg->seed) % 127) - 63;
        quants[x] = (int8_t) q;
        permuted_values[sb * 256 + x] = (float) q / 128.0f;
      }
    }
  } else {
    const size_t super_blocks = elements / 256;
    for (size_t sb = 0; sb < super_blocks; ++sb) {
      __fp16 *scales = (__fp16 *) (weight + sb * 144);
      uint8_t *packed = weight + sb * 144 + 16;
      uint8_t code[256];
      for (int group = 0; group < 8; ++group) scales[group] = (__fp16) (1.0f / 128.0f);
      for (int x = 0; x < 256; ++x) {
        code[x] = (uint8_t) ((sb * 7 + x * 5 + cfg->seed) & 15);
        permuted_values[sb * 256 + x] = iq4_values[code[x]] / 128.0f;
      }
      for (int x = 0; x < 64; ++x) {
        packed[2 * x] = code[x] | (code[x + 128] << 4);
        packed[2 * x + 1] = code[x + 64] | (code[x + 192] << 4);
      }
    }
  }
  for (int ki = 0; ki < k; ++ki)
    for (int nj = 0; nj < n; ++nj)
      logical_weight[(size_t) ki * n + nj] = permuted_values[audit_permuted_index(ki, nj, k)];
  memset(reference, 0, output_bytes);
  for (int r = 0; r < m; ++r) {
    /* Audit activations contain only 16 non-zero coordinates.  Keep ki as
     * the outer accumulation order so this is bit-for-bit equivalent to the
     * dense oracle while avoiding billions of known-zero multiply checks. */
    for (int ki = 0; ki < k; ++ki) {
      const float a = (float) (__fp16) activation[(size_t) r * k + ki];
      if (a == 0.0f) continue;
      for (int j = 0; j < n; ++j)
        reference[(size_t) r * n + j] += a * logical_weight[(size_t) ki * n + j];
    }
  }

  if (create_htp_message_channel(chan_fd, max_msg_size)) goto end;
  struct MatMulValueAuditParams params = {
    .output = { .fd = output_fd, .offset = 0 },
    .activation = { .fd = activation_fd, .offset = 0 },
    .weight = { .fd = weight_fd, .offset = 0 },
    .result = { .fd = result_fd, .offset = 0 },
    .m = m, .k = k, .n = n, .dtype = dtype, .requested_path = path,
    .seed = (int32_t) cfg->seed, .weight_bytes = (int64_t) weight_bytes,
  };
  struct MessageHeader *msg = (struct MessageHeader *) chan;
  const int request_ret = matmul_audit_send_request(msg, max_msg_size, &params);
  if (request_ret || result->status) {
    fprintf(stderr, "MATMUL_AUDIT_ERROR ret=%d status=%d\n", request_ret, result->status);
    goto release;
  }
  ...
  for (size_t i = 0; i < (size_t) m * n; ++i) {
    if (!isfinite(output[i])) { ++nonfinite; continue; }
    const double delta = (double) output[i] - reference[i];
    sq += delta * delta;
    ref_sq += (double) reference[i] * reference[i];
    if (fabs(delta) > max_abs) max_abs = (float) fabs(delta);
    if (fabs(reference[i]) > max_ref) max_ref = fabsf(reference[i]);
  }
  const double rmse = sqrt(sq / ((size_t) m * n));
  const double relative_l2 = sqrt(sq / fmax(ref_sq, 1e-30));
  const double normalized_max_abs = max_abs / fmax(max_ref, 1e-6f);
  const uint64_t host_activation_checksum = audit_checksum64(activation, activation_bytes);
  const uint64_t host_weight_checksum = audit_checksum64(weight, weight_bytes);
  const uint64_t host_output_checksum = audit_checksum64(output, output_bytes);
  const int checksums_match = host_activation_checksum == result->activation_checksum &&
    host_weight_checksum == result->weight_checksum && host_output_checksum == result->output_checksum;
  const int pass = nonfinite == 0 && relative_l2 <= 0.01 && normalized_max_abs <= 0.02 && checksums_match;
  printf("MATMUL_AUDIT_JSON {\"schema_version\":1,\"dtype\":\"%s\",\"requested_path\":\"%s\","
         "\"actual_path\":%d,\"m\":%d,\"k\":%d,\"n\":%d,\"seed\":%u,\"elapsed_us\":%lld,"
         "\"activation_checksum\":\"0x%016llx\",\"weight_checksum\":\"0x%016llx\","
         "\"output_checksum\":\"0x%016llx\",\"rmse\":%.9g,\"relative_l2\":%.9g,"
         "\"normalized_max_abs\":%.9g,\"nonfinite_count\":%d,\"stage_checksums_match\":%s,\"pass\":%s}\n",
         cfg->matmul_dtype, cfg->matmul_path, result->actual_path, m, k, n, cfg->seed,
         (long long) result->elapsed_us, (unsigned long long) result->activation_checksum,
         (unsigned long long) result->weight_checksum, (unsigned long long) result->output_checksum,
         rmse, relative_l2, normalized_max_abs, nonfinite,
         checksums_match ? "true" : "false", pass ? "true" : "false");
  ret = pass ? 0 : 3;
release:
  {
    int fds[] = { activation_fd, output_fd, weight_fd, result_fd };
    (void) figure8_release_dsp_maps(msg, max_msg_size, fds, 4);
  }
end:
  if (chan_fd >= 0) htp_ops_destroy_channel(get_global_handle());
  if (chan) free_shared_mem_buf(chan, chan_fd, max_msg_size);
  if (result) free_shared_mem_buf(result, result_fd, align_up(sizeof(*result), 128));
  if (weight) free_shared_mem_buf(weight, weight_fd, align_up(weight_bytes, 128));
  if (output) free_shared_mem_buf(output, output_fd, align_up(output_bytes, 128));
  if (activation) free_shared_mem_buf(activation, activation_fd, align_up(activation_bytes, 128));
  free(reference);
  free(permuted_values);
  free(logical_weight);
  return ret;
}

static int value_audit_distribution_id(const char *name) {
  if (strcmp(name, "exhaustive-fp16") == 0) return NONLINEAR_AUDIT_DISTRIBUTION_EXHAUSTIVE_FP16;
  if (strcmp(name, "dense") == 0) return NONLINEAR_AUDIT_DISTRIBUTION_DENSE;
  if (strcmp(name, "attention") == 0) return NONLINEAR_AUDIT_DISTRIBUTION_ATTENTION;
  return NONLINEAR_AUDIT_DISTRIBUTION_BOUNDARY;
}

static int compare_fp16_value(const void *lhs, const void *rhs) {
  const float a = (float) *(const __fp16 *) lhs;
  const float b = (float) *(const __fp16 *) rhs;
  return (a > b) - (a < b);
}

static void value_audit_fill_inputs(__fp16 *input, int elements, int distribution) {
  if (distribution == NONLINEAR_AUDIT_DISTRIBUTION_EXHAUSTIVE_FP16) {
    int out = 0;
    for (int bits = 0xfbff; bits >= 0x8000 && out < elements; --bits) {
      const uint16_t half_bits = (uint16_t) bits;
      memcpy(&input[out++], &half_bits, sizeof(half_bits));
    }
    return;
  }
  if (distribution == NONLINEAR_AUDIT_DISTRIBUTION_DENSE) {
    for (int i = 0; i < elements; ++i) input[i] = (__fp16) (-256.0f + 256.0f * i / (elements - 1));
    return;
  }
  if (distribution == NONLINEAR_AUDIT_DISTRIBUTION_ATTENTION) {
    uint32_t state = UINT32_C(0x53434e41);
    for (int i = 0; i < elements; ++i) {
      state = state * UINT32_C(1664525) + UINT32_C(1013904223);
      const float u = (float) (state & 0xffffu) / 65535.0f;
      input[i] = (__fp16) (-16.0f * u * u);
    }
    return;
  }

  static const float centers[] = {
    -256.0f, -16.0f, -13.7109375f, -11.421875f, -9.1484375f,
    -6.85546875f, -4.57421875f, -2.287109375f, 0.0f,
  };
  for (int i = 0; i < elements; ++i) {
    __fp16 center = (__fp16) centers[(i / 5) % (int) (sizeof(centers) / sizeof(centers[0]))];
    uint16_t bits;
    memcpy(&bits, &center, sizeof(bits));
    const int neighbor = i % 5 - 2;
    int adjusted = (int) bits + neighbor;
    if (adjusted < 0x8000) adjusted = 0x8000;
    if (adjusted > 0xfbff) adjusted = 0xfbff;
    const uint16_t half_bits = (uint16_t) adjusted;
    memcpy(&input[i], &half_bits, sizeof(half_bits));
  }
  qsort(input, (size_t) elements, sizeof(*input), compare_fp16_value);
}

static void value_audit_topk(const __fp16 *values, int elements, int *indices, int k) {
  for (int j = 0; j < k; ++j) indices[j] = -1;
  for (int i = 0; i < elements; ++i) {
    const float v = (float) values[i];
    int pos = k;
    for (int j = 0; j < k; ++j) {
      if (indices[j] < 0 || v > (float) values[indices[j]]) { pos = j; break; }
    }
    if (pos < k) {
      for (int j = k - 1; j > pos; --j) indices[j] = indices[j - 1];
      indices[pos] = i;
    }
  }
}

static int run_value_audit(const struct Figure8AttnConfig *cfg) {
  const int distribution = value_audit_distribution_id(cfg->lut_distribution);
  const int elements = distribution == NONLINEAR_AUDIT_DISTRIBUTION_EXHAUSTIVE_FP16
      ? 31744 : cfg->value_audit_elements;
  const int evaluator = strcmp(cfg->mode, "lut-exp") == 0
      ? NONLINEAR_AUDIT_EVALUATOR_LUT_EXP : NONLINEAR_AUDIT_EVALUATOR_SCNA_FP16;
  const size_t data_size = (size_t) align_up((size_t) elements * sizeof(__fp16), 128);
  const size_t result_size = (size_t) align_up(sizeof(struct NonlinearValueAuditResult), 128);
  const size_t max_msg_size = 4096;
  __fp16 *input = NULL, *output = NULL;
  struct NonlinearValueAuditResult *result = NULL;
  void *chan = NULL;
  int input_fd = -1, output_fd = -1, result_fd = -1, chan_fd = -1, ret = 1;
  if (alloc_shared_mem_buf((void **) &input, &input_fd, data_size) ||
      alloc_shared_mem_buf((void **) &output, &output_fd, data_size) ||
      alloc_shared_mem_buf((void **) &result, &result_fd, result_size) ||
      alloc_shared_mem_buf(&chan, &chan_fd, max_msg_size)) goto end;
  memset(output, 0, data_size);
  memset(result, 0, result_size);
  value_audit_fill_inputs(input, elements, distribution);
  if (create_htp_message_channel(chan_fd, max_msg_size)) goto end;
  struct NonlinearValueAuditParams params = {
    .input = { .fd = input_fd, .offset = 0 },
    .output = { .fd = output_fd, .offset = 0 },
    .result = { .fd = result_fd, .offset = 0 },
    .elements = elements,
    .evaluator = evaluator,
  ...
  struct MessageHeader *msg = (struct MessageHeader *) chan;
  const int request_ret = value_audit_send_request(msg, max_msg_size, &params);
  if (request_ret) {
    fprintf(stderr, "VALUE_AUDIT_ERROR evaluator=%s distribution=%s ret=%d\n",
            cfg->mode, cfg->lut_distribution, request_ret);
    goto end;
  }

  double ref_sum = 0.0, actual_sum = 0.0;
  for (int i = 0; i < elements; ++i) {
    ref_sum += exp2((double) (float) input[i]);
    actual_sum += (double) (float) output[i];
  }
  double tv = 0.0, kl = 0.0, actual_probability_sum = 0.0;
  const double epsilon = 1.0e-30;
  if (ref_sum > 0.0 && actual_sum > 0.0) {
    for (int i = 0; i < elements; ++i) {
      const double p = exp2((double) (float) input[i]) / ref_sum;
      const double q = (double) (float) output[i] / actual_sum;
      actual_probability_sum += q;
      tv += fabs(p - q);
      kl += p * log((p + epsilon) / (q + epsilon));
    }
    tv *= 0.5;
  }
  int reference_top[10], actual_top[10];
  __fp16 *reference_values = (__fp16 *) malloc((size_t) elements * sizeof(__fp16));
  int topk_overlap = 0;
  if (reference_values) {
    for (int i = 0; i < elements; ++i) reference_values[i] = (__fp16) exp2f((float) input[i]);
    value_audit_topk(reference_values, elements, reference_top, 10);
    value_audit_topk(output, elements, actual_top, 10);
    for (int i = 0; i < 10; ++i)
      for (int j = 0; j < 10; ++j)
        if (reference_top[i] == actual_top[j]) { ++topk_overlap; break; }
    free(reference_values);
  }
  fprintf(stdout,
          "VALUE_AUDIT_JSON {\"schema_version\":%d,\"evaluator\":\"%s\",\"build_flavor\":%d,"
          "\"distribution\":\"%s\",\"elements\":%d,\"iters\":%d,\"elapsed_ticks\":%lld,"
          "\"elapsed_us\":%lld,\"ns_per_element\":%.9g,\"checksum\":\"0x%016llx\","
          "\"rmse\":%.9g,\"max_abs_error\":%.9g,\"mean_abs_error\":%.9g,\"relative_l2\":%.9g,"
          "\"nonfinite_count\":%d,\"negative_count\":%d,\"monotonic_violations\":%d,"
          "\"softmax_probability_sum\":%.17g,\"softmax_tv\":%.17g,\"softmax_kl\":%.17g,"
          "\"top10_overlap\":%d,\"vtcm_total_bytes\":%lld,\"vtcm_reserved_bytes\":%lld,"
          "\"vtcm_seq_capacity_bytes\":%lld,\"lut_table_bytes\":%lld,\"lut_init_us\":%lld}\n",
          result->schema_version, cfg->mode, result->build_flavor, cfg->lut_distribution, result->elements,
          result->iters, (long long) result->elapsed_ticks, (long long) result->elapsed_us,
          result->elements > 0 && result->iters > 0 ? 1000.0 * result->elapsed_us / result->elements / result->iters : 0.0,
          (unsigned long long) result->checksum, result->rmse, result->max_abs_error, result->mean_abs_error,
          result->relative_l2, result->nonfinite_count, result->negative_count, result->monotonic_violations,
          actual_probability_sum, tv, kl, topk_overlap, (long long) result->vtcm_total_bytes,
          (long long) result->vtcm_reserved_bytes, (long long) result->vtcm_seq_capacity_bytes,
          (long long) result->lut_table_bytes, (long long) result->lut_init_us);
  {
    int fds[] = { input_fd, output_fd, result_fd };
    (void) figure8_release_dsp_maps(msg, max_msg_size, fds, 3);
  }
  ret = 0;
end:
  if (chan_fd >= 0) htp_ops_destroy_channel(get_global_handle());
  if (chan) free_shared_mem_buf(chan, chan_fd, max_msg_size);
  if (result) free_shared_mem_buf(result, result_fd, result_size);
  if (output) free_shared_mem_buf(output, output_fd, data_size);
  if (input) free_shared_mem_buf(input, input_fd, data_size);
  return ret;
}
...
static int run_scna_exp_benchmark(const struct Figure8AttnConfig *cfg) {
  ...
  struct ScnaExp2BenchParams params = {
    ...
    .layout = SCNA_LAYOUT_SERIAL,
    .variant = scna_variant_id(cfg->scna_variant),
    .warmup = cfg->warmup,
    .iters = cfg->iters,
  };
  struct MessageHeader *msg = (struct MessageHeader *) chan;
  ...
  fprintf(stderr,
          "SCNA_EXP_BENCH schema_version=%d kernel_impl=%d layout=%s variant=%s variant_id=%d build_variant=%d optimized_inline=%d optimized_impl=%d dead_neurons_removed=%d width=%d lanes=%d warmup=%d iters=%d prepare_elapsed_us=%lld param_prepare_ns=%.6f elapsed_us=%lld "
          "single_ns_per_64=%.6f pair_elapsed_us=%lld paired_ns_per_2x64=%.6f paired_ns_per_64=%.6f quad_elapsed_us=%lld quad_ns_per_4x64=%.6f quad_ns_per_64=%.6f "
          ...
          "random_samples=%d random_rmse=%.9g random_max_abs=%.9g random_nonfinite_count=%d "
          "pair_max_abs_diff=%.9g monotonic_violations=%d negative_count=%d nan_count=%d "
          "lane_oracle_mismatches=%d canonical_oracle_mismatches=%d paired_single_mismatches=%d quad_pair_mismatches=%d "
          ...
          "scale_head_dim=%d scale_fp16_bits=0x%04x scale_only_elapsed_us=%lld scale_only_ns_per_64=%.6f "
          "separate_scale_lut_elapsed_us=%lld separate_scale_lut_ns_per_64=%.6f "
          "separate_scale_scna_elapsed_us=%lld separate_scale_scna_ns_per_64=%.6f "
          "fused_scale_scna_elapsed_us=%lld fused_scale_scna_ns_per_64=%.6f "
          "fused_scale_max_abs_diff=%.9g fused_scale_mismatches=%d fused_scale_samples=%d "
          "scale_checksum=0x%08x\n",
          result->schema_version, result->kernel_impl,
          ...
          paired_ns * 0.5, (long long) result->quad_elapsed_us,
          result->iters > 0 ? 1000.0 * result->quad_elapsed_us / result->iters : 0.0,
          result->iters > 0 ? 250.0 * result->quad_elapsed_us / result->iters : 0.0,
          ...
          result->dense_max_abs_error, result->random_samples, result->random_rmse,
          result->random_max_abs_error, result->random_nonfinite_count,
          result->pair_max_abs_diff, result->monotonic_violations,
          ...
          result->scale_head_dim, result->scale_fp16_bits,
          (long long) result->scale_only_elapsed_us,
          result->iters > 0 ? 500.0 * result->scale_only_elapsed_us / result->iters : 0.0,
          (long long) result->separate_scale_lut_elapsed_us,
          result->iters > 0 ? 500.0 * result->separate_scale_lut_elapsed_us / result->iters : 0.0,
          (long long) result->separate_scale_scna_elapsed_us,
          result->iters > 0 ? 500.0 * result->separate_scale_scna_elapsed_us / result->iters : 0.0,
          (long long) result->fused_scale_scna_elapsed_us,
          result->iters > 0 ? 500.0 * result->fused_scale_scna_elapsed_us / result->iters : 0.0,
          result->fused_scale_max_abs_diff, result->fused_scale_mismatches,
          result->fused_scale_samples, result->scale_checksum_bits);
  {
    int fd = result_fd;
...
static int run_figure8_attn_benchmark(const struct Figure8AttnConfig *cfg) {
  ...
  const int    profile_max_records = ceil_div_int(cfg->qo_len, 4) * cfg->n_kv_heads + 8;
  ...
  int          profile_max_events  = cfg->print_events ? cfg->n_kv_heads * max_q_blocks * (4 + max_k_blocks * 8) + 128 : 0;
  if (cfg->print_events && profile_max_events < 1024) {
  ...
  if (cfg->csv_out) {
    ...
    fprintf(csv, "mode,variant,workers,q_task_rows,qo_len,kv_len,n_heads,n_kv_heads,head_dim,phase,iteration,host_elapsed_us,ret\n");
  }
  ...
  if (strcmp(cfg->mode, "scna-fp16") == 0) {
    ...
    mode_flags |= scna_variant_id(cfg->scna_variant) << 10;
  }
  mode_flags |= cfg->workers << LLM_NPU_MODE_WORKER_COUNT_SHIFT;
  int q_task_rows_code = 0;
  if (cfg->q_task_rows == 8) q_task_rows_code = 1;
  else if (cfg->q_task_rows == 16) q_task_rows_code = 2;
  else if (cfg->q_task_rows == 32) q_task_rows_code = 3;
  mode_flags |= q_task_rows_code << LLM_NPU_MODE_Q_TASK_ROWS_SHIFT;
  if (cfg->q_task_rows == 0) mode_flags |= LLM_NPU_MODE_Q_TASK_ROWS_AUTO;
  ...
  fprintf(stderr,
          ...
          "profile_max_events=%d mode_flags=%d print_events=%d workers=%d q_task_rows=%d\n",
          cfg->mode, cfg->scna_layout, cfg->scna_variant, cfg->scna_width, cfg->seed_label,
          ...
          cfg->print_events, cfg->workers, cfg->q_task_rows);
  ...
  for (int i = 0; i < cfg->warmup + cfg->iters; ++i) {
    ...
    fprintf(stderr,
            "FIG8_ATTENTION_HOST_TIMING mode=%s layout=%s variant=%s workers=%d q_task_rows=%d scna_width=%d seed=%s mask_mode=%s qo_len=%d kv_len=%d "
            "n_heads=%d n_kv_heads=%d head_dim=%d phase=%s "
            "iteration=%d host_elapsed_us=%ld ret=%d\n",
            cfg->mode, cfg->scna_layout, cfg->scna_variant, cfg->workers, cfg->q_task_rows, cfg->scna_width, cfg->seed_label, cfg->mask_mode, cfg->qo_len, cfg->kv_len, cfg->n_heads,
            cfg->n_kv_heads, cfg->head_dim, phase,
            is_warmup ? i : measured_idx, elapsed, req_ret);
    if (csv) {
      fprintf(csv, "%s,%s,%d,%d,%d,%d,%d,%d,%d,%s,%d,%ld,%d\n", cfg->mode, cfg->scna_variant, cfg->workers, cfg->q_task_rows,
    ...
    fprintf(stderr,
            "FIG8_ATTENTION_WORKERS mode=%s variant=%s qo_len=%d phase=%s iteration=%d requested_workers=%d "
            "active_workers=%d hvx_contexts=%d vtcm_worker_cap=%d tasks=%d q_task_rows=%d\n",
            cfg->mode, cfg->scna_variant, cfg->qo_len, phase, is_warmup ? i : measured_idx,
            cfg->workers, profile_hdr->active_workers, profile_hdr->hvx_contexts,
            profile_hdr->vtcm_worker_cap, profile_hdr->tasks, profile_hdr->q_task_rows);
    fprintf(stderr,
            "FIG8_AUDIT_RESOURCE schema_version=%d mode=%s phase=%s iteration=%d "
            "vtcm_total_bytes=%lld vtcm_reserved_bytes=%lld vtcm_seq_capacity_bytes=%lld "
            "lut_table_bytes=%lld lut_init_us=%lld active_workers=%d tasks=%d q_task_rows=%d\n",
            profile_hdr->schema_version, cfg->mode, phase, is_warmup ? i : measured_idx,
            (long long) profile_hdr->vtcm_total_bytes, (long long) profile_hdr->vtcm_reserved_bytes,
            (long long) profile_hdr->vtcm_seq_capacity_bytes, (long long) profile_hdr->lut_table_bytes,
            (long long) profile_hdr->lut_init_us, profile_hdr->active_workers, profile_hdr->tasks,
            profile_hdr->q_task_rows);
    ...
    for (int r = 0; r < record_count; ++r) {
      ...
      fprintf(stderr,
              ...
              "qk_dot=%ld safe_sm=%ld scna_exp=%ld state_update=%ld rowmax_mem=%ld rowsum_reduce=%ld "
              "rowmax_mem_ticks=%ld rowsum_reduce_ticks=%ld state_update_ticks=%ld "
              "param_prepare=%ld core_acc=%ld o_scale=%ld o_store=%ld\n",
              ...
              rec->k_load, rec->v_load, rec->qk_dot, rec->safe_sm, rec->scna_exp, rec->state_update,
              rec->rowmax_mem, rec->rowsum_reduce,
              rec->rowmax_mem_ticks, rec->rowsum_reduce_ticks, rec->state_update_ticks,
              rec->param_prepare,
              rec->core_acc, rec->o_scale,
              rec->o_store);
      if (cfg->numeric_debug) {
...
int main(int argc, char **argv) {
  const int64_t process_main_t0 = get_time_us();
  ...
  const int64_t session_t0 = get_time_us();
  int err = open_dsp_session(CDSP_DOMAIN_ID, 1);
  const int64_t session_open_us = get_time_us() - session_t0;
  if (err != 0) {
    fprintf(stderr, "Open DSP session failed\n");
    return 1;
  }

  const int64_t backend_t0 = get_time_us();
  init_htp_backend();
  const int64_t backend_init_us = get_time_us() - backend_t0;
  fprintf(stderr,
          "AUDIT_STARTUP_TIMING session_open_us=%lld backend_init_us=%lld main_to_ready_us=%lld\n",
          (long long) session_open_us, (long long) backend_init_us,
          (long long) (get_time_us() - process_main_t0));

  if (figure8_cfg.matmul_audit) {
    int audit_ret = run_matmul_value_audit(&figure8_cfg);
    close_dsp_session();
    return audit_ret;
  }

  if (figure8_cfg.value_audit) {
    int audit_ret = run_value_audit(&figure8_cfg);
    close_dsp_session();
    return audit_ret;
  }
