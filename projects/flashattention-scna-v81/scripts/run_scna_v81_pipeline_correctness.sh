...
OUT_DIR="${OUT_DIR:-results/v81/scna/pipeline-correctness-$(date +%Y%m%d-%H%M%S)}"
...
complete_log() {
  local log_path="$1"
  [[ -s "$log_path" ]] || return 1
  grep -Eq '^FIG8_ATTENTION_PIPELINE_COMPARE .*gate=pass ret=0$' "$log_path" || return 1
  grep -Eq '^FIG8_ATTENTION_COMPARE .*reference_mode=host-fp32 .*ret=0$' "$log_path"
}
      ...
      for width in 8 16 32; do
        for case_spec in "${cases[@]}"; do
          ...
          run_logged "$OUT_DIR/raw/${stem}.log" \
            ...
            --scna-width "$width" --scna-pipeline on --mask-mode "$mask" --qo-len 4 --kv-len "$kv_len" \
            --n-heads 12 --n-kv-heads 2 --head-dim "$head_dim" --warmup 1 --iters 1 --no-events \
            --numeric-debug --compare-pipeline --compare-reference
        done
      done
    done
  done
...
python3 scripts/analyze_scna_pipeline_correctness.py --input "$OUT_DIR/raw/all.log" --out-dir "$OUT_DIR/summary"
printf 'SCNA v81 pipeline correctness complete: %s\n' "$OUT_DIR"
