...
const NPU_MODES = [
  ...
  { id: 'lut_exp', label: 'LUT-EXP' },
];
const DEFAULT_NPU_MODE = 'baseline';
...
const StorageUtils = {
  ...
  getNpuMode() {
    const saved = localStorage.getItem('npu_mode') || DEFAULT_NPU_MODE;
    return NPU_MODES.some(mode => mode.id === saved) ? saved : DEFAULT_NPU_MODE;
  },
  setNpuMode(mode) {
    localStorage.setItem('npu_mode', mode);
  },
};
...
const mainApp = createApp({
  ...
  data() {
    return {
      ...
      lastSpeedStats: null,
      selectedNpuMode: StorageUtils.getNpuMode(),
  ...
  methods: {
    ...
    setNpuMode(mode) {
      if (this.isGenerating || this.selectedNpuMode === mode) return;
      this.selectedNpuMode = mode;
      this.lastGenerationMode = null;
      this.lastSpeedStats = null;
      StorageUtils.setNpuMode(mode);
    },
    newConversation() {
    ...
    async generateMessage(currConvId) {
      ...
      try {
        ...
        const params = {
          ...
          stream: true,
          cache_prompt: this.lastGenerationMode === this.selectedNpuMode,
          npu_mode: this.selectedNpuMode,
        ...
        for await (const chunk of chunks) {
          ...
          if (timings) {
            const timingStats = this.extractSpeedStats(timings);
            this.lastSpeedStats = timingStats || this.lastSpeedStats;
        ...
        this.lastGenerationMode = this.selectedNpuMode;
    ...
    extractSpeedStats(timings) {
      if (!timings) return null;

      const promptPerSecond = Number.isFinite(timings.prompt_per_second)
        ? timings.prompt_per_second
        : timings.prompt_n / (timings.prompt_ms / 1000);
      const predictedPerSecond = Number.isFinite(timings.predicted_per_second)
        ? timings.predicted_per_second
        : timings.predicted_n / (timings.predicted_ms / 1000);

      return {
        prompt_per_second: Number.isFinite(promptPerSecond) ? promptPerSecond : null,
        predicted_per_second: Number.isFinite(predictedPerSecond) ? predictedPerSecond : null,
        prompt_n: timings.prompt_n,
        predicted_n: timings.predicted_n,
      };
    },
    ...
    updateLatestSpeedStatsFromMessages() {
      const lastTimedMsg = [...this.messages].reverse().find(msg => msg.timings);
      this.lastSpeedStats = lastTimedMsg ? this.extractSpeedStats(lastTimedMsg.timings) : null;
    },
    ...
    fetchMessages() {
      ...
      this.updateLatestSpeedStatsFromMessages();
    },
