...
static int64_t g_audit_lut_init_us = 0;
static int64_t g_audit_lut_table_bytes = 0;

static __attribute__((unused)) void precompute_safe_softmax_exp2_table() {
  const int n_dup = 4;
  ...
  g_audit_lut_init_us = elapsed_us;
  g_audit_lut_table_bytes = 65536LL * n_dup;
...
void init_precomputed_tables() {
#if AUDIT_BUILD_FLAVOR != 2
  precompute_safe_softmax_exp2_table();
#else
  g_audit_lut_init_us = 0;
  g_audit_lut_table_bytes = 0;
  FARF(ALWAYS, "Stage2.9 scna_only: LUT allocation and initialization disabled");
#endif
}

int64_t audit_lut_init_us(void) { return g_audit_lut_init_us; }
int64_t audit_lut_table_bytes(void) { return g_audit_lut_table_bytes; }
