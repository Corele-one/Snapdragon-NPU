...
#include "dsp/flash_attn_rowsum.h"
...
void simple_flash_attn_f16_core(int kv_head_idx, uint8_t *vtcm, uint8_t *vtcm_limit, __fp16 *restrict O,
  ...
  uint8_t *vtcm_exp2_table = vtcm_exp2_table_base != NULL
      ? vtcm_exp2_table_base + sub_table_idx * sub_table_size
      : NULL;
  assert(!enable_vgather_exp || vtcm_exp2_table != NULL);
      ...
      int64_t safe_sm_stage_t0 = HAP_perf_get_qtimer_count();
      {
        ...
        for (int r_vec_idx = 0; r_vec_idx < n_row_vec_cnt; ++r_vec_idx) {
          ...
          for (int r_vec_off = 0; r_vec_off < 64; r_vec_off += 2) {
            ...
            float            p_rowsum0_f32 = 0.0f;
            float            p_rowsum1_f32 = 0.0f;
...
#define SCNA_CONSUME_P_BLOCK(c_value, p_row0_value, p_row1_value, score_row0_value) do { \
              ...
              p_rowsum0_f32 += flash_attn_sum_vhf(v_p_row0_consume); \
              p_rowsum1_f32 += flash_attn_sum_vhf(v_p_row1_consume); \
...
#endif
            HVX_Vector v_p_rowsum_pack2 = flash_attn_pack_rowsums_f32(p_rowsum0_f32, p_rowsum1_f32);
...
static int simple_flash_attn_impl(__fp16 *restrict O, const __fp16 *restrict Q, const __fp16 *restrict K,
  ...
  const int q_task_rows_code = (mode_flags & LLM_NPU_MODE_Q_TASK_ROWS_MASK) >> LLM_NPU_MODE_Q_TASK_ROWS_SHIFT;
  const bool q_task_rows_auto = (mode_flags & LLM_NPU_MODE_Q_TASK_ROWS_AUTO) != 0;
  /* Profiling on this device selected two query chunks per KV head for q>=8:
   * q4->4, q8->4, q16->8, q32->16. This exposes four tasks without the 8x
   * K/V reload multiplier of the old fixed-r4 scheduler. */
  const int q_task_rows = q_task_rows_auto
      ? (qo_len <= 8 ? 4 : (qo_len <= 16 ? 8 : 16))
      : (4 << q_task_rows_code);
