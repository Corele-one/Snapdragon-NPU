...
static int transfer_permuted_weight_chunk_fp16(__fp16 *vtcm_dst, const __fp16 *src, int n_cols, int k) {
  ...
  if (use_dma) {
    ...
    /* The DMA engine requires a 64-byte aligned descriptor in static storage.
     * Model graph execution can submit matmuls concurrently, so ordinary
     * function-static storage races.  TLS gives every RPC thread a persistent,
     * independently aligned descriptor. */
    static _Thread_local dma_desc_1d_t _Alignas(64) desc;
    const int dma_ret = dma_issue_load_from_ddr(&desc, vtcm_dst, src, size);
    if (dma_ret != 0) {
      FARF(ALWAYS, "%s: FP16 weight DMA submit failed: ret=%d bytes=%u", __func__, dma_ret, (unsigned) size);
      return dma_ret;
    }
    dma_wait_for_idle();
  ...
  for (size_t mr = 0; mr < m; mr += m_chunk_n_rows) {
    ...
    for (size_t nc = 0; nc < n; nc += n_chunk_n_cols) {
      ...
      int64_t wei_t0_us = matmul_trace_now_us();
      {
        ...
        if (transfer_permuted_weight_chunk_fp16(vtcm_weight, permuted_weight_chunk, n_cols, k) != 0) {
          return -1;
        }
      }
