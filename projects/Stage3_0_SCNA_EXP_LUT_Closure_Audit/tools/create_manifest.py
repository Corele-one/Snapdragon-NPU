#!/usr/bin/env python3
"""Create deterministic source/artifact provenance for the Stage 3.0 audit."""
...
def main() -> int:
    ...
    for flavor in ("exp_lut_system", "scna_system", "fair_combined", "lut_only", "scna_only"):
    ...
    if model_host_dir.is_dir():
        for path in sorted(p for p in model_host_dir.iterdir() if p.is_file()):
            model_host_artifacts[path.name] = {"bytes": path.stat().st_size, "sha256": sha256(path),
                                               "build_id": artifact_build_id(path)}
    llama_bridge = project / "work/llama.cpp-stage30/ggml/src/ggml-htp"
    ...
    for name, root in {
        "exp_lut_system": project / "systems/exp_lut_system/htp-ops-lib-main",
        "scna_system": project / "systems/scna_system/htp-ops-lib-main",
    }.items():
        digest, files = tree_digest(root)
        manifest["system_sources"][name] = {"path": str(root), "tree_sha256": digest, "files": files}
