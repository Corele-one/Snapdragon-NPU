...
usage() { echo "Usage: $0 [--flavor exp_lut_system|scna_system|fair_combined|lut_only|scna_only] [--remote-dir PATH]" >&2; }
while [[ $# -gt 0 ]]; do
  case "$1" in
...
case "$flavor" in exp_lut_system|scna_system|fair_combined|lut_only|scna_only) ;; *) usage; exit 2 ;; esac

host_dir="$project_dir/artifacts/host"
dsp_dir="$project_dir/artifacts/$flavor"
if [[ -f "$dsp_dir/htp_ops_test" ]]; then
  host_dir="$dsp_dir"
fi
