...
dsp_arch="v79"
variant="pair_static_d8"
optimized_inline=0
optimized_impl="fma"
kernel_impl="d7_pairret_noinline"
softmax_impl="baseline"
fine_timers=0
stage25_flags=0
dsp_only=0

usage() {
  echo "Usage: $0 [--variant NAME] [--kernel-impl NAME] [--softmax-impl NAME] [--dsp-only]  # v79 only" >&2
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --softmax-impl)
      [[ $# -ge 2 ]] || { usage; exit 2; }
      softmax_impl="$2"; shift 2 ;;
    --fine-timers)
      [[ $# -ge 2 && ( "$2" == 0 || "$2" == 1 ) ]] || { usage; exit 2; }
      fine_timers="$2"; shift 2 ;;
    --stage25-flags)
      [[ $# -ge 2 && "$2" =~ ^[0-9]+$ ]] || { usage; exit 2; }
      stage25_flags="$2"; shift 2 ;;
    --dsp-only)
      dsp_only=1; shift ;;
    --help|-h)
      usage
...
case "$softmax_impl" in
  baseline) softmax_impl_id=0 ;;
  tail_s_resident) softmax_impl_id=1 ;;
  fused_state_update) softmax_impl_id=2 ;;
  fused_state_unroll2) softmax_impl_id=3 ;;
  *) echo "Unknown Softmax implementation: $softmax_impl" >&2; exit 2 ;;
esac
...
build_cmake hexagon "DSP_ARCH=$dsp_arch" \
  ...
  "STAGE2_SOFTMAX_IMPL=$softmax_impl_id" \
  "STAGE2_ENABLE_FINE_TIMERS=$fine_timers" \
  "STAGE25_CLEANUP_FLAGS=$stage25_flags"

echo "Built variant=$variant build_id=$variant_id kernel_impl=$kernel_impl($kernel_impl_id) softmax_impl=$softmax_impl($softmax_impl_id) stage25_flags=$stage25_flags for $dsp_arch."
