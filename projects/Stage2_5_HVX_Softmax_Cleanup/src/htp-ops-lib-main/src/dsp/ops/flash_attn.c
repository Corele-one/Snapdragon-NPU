...
#ifndef STAGE2_SOFTMAX_IMPL
#  define STAGE2_SOFTMAX_IMPL 0
#endif

#ifndef STAGE2_ENABLE_FINE_TIMERS
#  define STAGE2_ENABLE_FINE_TIMERS 0
#endif

#ifndef STAGE25_CLEANUP_FLAGS
#  define STAGE25_CLEANUP_FLAGS 0
#endif

#define STAGE25_A_MASK_INDEX          1
#define STAGE25_B_FULL_MASK_FASTPATH  2
#define STAGE25_C_ALIGNED_MASK_LOAD   4
#define STAGE25_D_INVARIANT_HOIST     8
#define STAGE25_E_TASK_GEOMETRY      16
#define STAGE25_F_ROUNDTRIP          32
...
void simple_flash_attn_f16_core(int kv_head_idx, uint8_t *vtcm, uint8_t *vtcm_limit, __fp16 *restrict O,
                                ...
                                int head_dim, int worker_index, int mode_flags,
                                const scna_exp2_hvx_params_t *prepared_scna_params,
                                struct Figure8ProfileHeader *profile,
  ...
  const bool full_mask_fastpath =
    ((STAGE25_CLEANUP_FLAGS & STAGE25_B_FULL_MASK_FASTPATH) != 0) &&
    ((mode_flags & LLM_NPU_MODE_MASK_FULL_FASTPATH) != 0);
  const bool use_fp32_exp       = false;
  const scna_exp2_hvx_params_t *scna_hvx_params = prepared_scna_params;
  assert(!enable_scna_exp || (scna_hvx_params != NULL && scna_hvx_params->width == 8));
#if (STAGE25_CLEANUP_FLAGS & STAGE25_C_ALIGNED_MASK_LOAD) != 0
  assert(is_aligned(qk_mask, VLEN));
#endif
...
#if STAGE2_SOFTMAX_IMPL == 2 || STAGE2_SOFTMAX_IMPL == 3
  (void) mvec_p_rowsum;
#endif
  ...
  int64_t state_update_ticks = 0;
  int64_t rowmax_mem_ticks = 0;
  int64_t rowsum_reduce_ticks = 0;
  ...
  for (int ir = 0; ir < qo_len; ir += blk_sz_r) {
    ...
    int64_t q_load_stage_t0 = HAP_perf_get_qtimer_count();
    {
...
#if (STAGE25_CLEANUP_FLAGS & STAGE25_A_MASK_INDEX) != 0
      int q_idx0_incremental = 0;
      int h_idx0_incremental = 0;
#endif
      for (int r = 0; r < n_rows_g; r += 2) {
        ...
        // input positions
#if (STAGE25_CLEANUP_FLAGS & STAGE25_A_MASK_INDEX) != 0
        int q_idx0 = q_idx0_incremental;
        int h_idx0 = h_idx0_incremental;
        int q_idx1 = q_idx0;
        int h_idx1 = h_idx0 + 1;
        if (h_idx1 >= G) {
          h_idx1 = 0;
        ...
        // clang-format on
#if (STAGE25_CLEANUP_FLAGS & STAGE25_A_MASK_INDEX) != 0
        q_idx0_incremental = q_idx1;
        h_idx0_incremental = h_idx1 + 1;
        if (h_idx0_incremental >= G) {
          h_idx0_incremental = 0;
          ++q_idx0_incremental;
        }
#endif
      }
...
#if (STAGE25_CLEANUP_FLAGS & STAGE25_D_INVARIANT_HOIST) != 0
        const HVX_Vector v_fp16_mask_threshold_hoisted = Q6_Vh_vsplat_R(0xcc00);  // fp16: -16.0
        const HVX_Vector v_state_sign_hoisted = Q6_Vh_vsplat_R(0x8000);
#endif
...
#if STAGE2_ENABLE_FINE_TIMERS
            TIMER_START(rowmax_mem);
#endif
...
#if STAGE2_SOFTMAX_IMPL == 1
            HVX_Vector v_s_tail0 = v_neg_inf;
            HVX_Vector v_s_tail1 = v_neg_inf;
            int        scna_tail_c = 0;
#endif
#if (STAGE25_CLEANUP_FLAGS & STAGE25_A_MASK_INDEX) != 0
            const int q_idx0_mask = ir + (r + 0) / G;
            const int q_idx1_mask = ir + (r + 1) / G;
#endif
            // reduction phase 1: inter-vector
            for (int c = 0; c < n_cols; c += 64) {
#if (STAGE25_CLEANUP_FLAGS & STAGE25_A_MASK_INDEX) != 0
              const int q_idx0 = q_idx0_mask;
              const int q_idx1 = q_idx1_mask;
#else
              int q_idx0 = ir + (r + 0) / G;
              int q_idx1 = ir + (r + 1) / G;
#endif
              ...
              if (full_mask_fastpath) {
                const size_t ne = smin(n_cols - c, 64);
                q_mask_keep0 = q_mask_keep1 = Q6_Q_vsetq2_R(ne * sizeof(__fp16));
              } else if (has_qk_mask) {
#if (STAGE25_CLEANUP_FLAGS & STAGE25_C_ALIGNED_MASK_LOAD) != 0
                HVX_Vector v_mask0 = vmem(qk_mask + q_idx0 * kv_pad_len + k_idx);
                HVX_Vector v_mask1 = vmem(qk_mask + q_idx1 * kv_pad_len + k_idx);
#else
...
#if (STAGE25_CLEANUP_FLAGS & STAGE25_D_INVARIANT_HOIST) != 0
                const HVX_Vector v_fp16_mask_threshold = v_fp16_mask_threshold_hoisted;
#else
...
#if STAGE2_SOFTMAX_IMPL == 1
              if (enable_scna_exp && c + 64 >= n_cols) {
                v_s_tail0 = v_s_row0;
                v_s_tail1 = v_s_row1;
                scna_tail_c = c;
              } else {
...
#if SCNA_KERNEL_IMPL == SCNA_KERNEL_IMPL_D7_PREBROADCAST
            const scna_exp2_prebroadcast_t scna_prebroadcast =
              scna_prebroadcast_prepare_inline(scna_hvx_params);
#endif
#define SCNA_CONSUME_P_BLOCK(c_value, p_row0_value, p_row1_value, score_row0_value) do { \
              const int scna_c = (c_value); \
              const HVX_Vector v_p_row0_consume = (p_row0_value); \
              const HVX_Vector v_p_row1_consume = (p_row1_value); \
              if (numeric_debug && kv_head_idx == 0 && ir == 0 && jc + n_cols >= kv_len && r == 0 && \
                  scna_c + 64 >= n_cols) { \
                _Alignas(VLEN) __fp16 debug_p[64]; \
                vmem(debug_p) = v_p_row0_consume; \
              ...
              if (numeric_debug && kv_head_idx == 0 && ir == 0 && r == 0) { \
                ...
                vmem(debug_p) = v_p_row0_consume; \
                vmem(debug_score) = (score_row0_value); \
                vmem(debug_m) = v_dup_m0; \
                for (int lane = 0; lane < 64; ++lane) { \
                  const int k_index = jc + scna_c + lane; \
              ...
              __fp16 *out_dual_tile = p_st_base + (scna_c / 64) * HMX_FP16_TILE_N_ELMS * 2; \
              HVX_Vector *pv_p_out0 = ((HVX_Vector *) out_dual_tile) + r1 / 2; \
              HVX_Vector *pv_p_out1 = pv_p_out0 + 16; \
              const HVX_VectorPair vp_p_dual_row = \
                Q6_W_vshuff_VVR(v_p_row1_consume, v_p_row0_consume, -2); \
              *pv_p_out0 = Q6_V_lo_W(vp_p_dual_row); \
              *pv_p_out1 = Q6_V_hi_W(vp_p_dual_row); \
              const HVX_VectorPair vp_p_row0 = hvx_my_vhf_to_wqf32(v_p_row0_consume); \
              const HVX_VectorPair vp_p_row1 = hvx_my_vhf_to_wqf32(v_p_row1_consume); \
    ...
    SCNA_KERNEL_IMPL == SCNA_KERNEL_IMPL_COMBINED_CONFIRM
            if (enable_scna_exp) {
              ...
              for (; scna_c_begin + 64 < n_cols; scna_c_begin += 128) {
                const int c0 = scna_c_begin;
                const int c1 = scna_c_begin + 64;
                HVX_Vector x00 = Q6_Vhf_vadd_VhfVhf(row_buffer0[c0 / 64], Q6_V_vxor_VV(v_dup_m0, v_sign));
                HVX_Vector x01 = Q6_Vhf_vadd_VhfVhf(row_buffer1[c0 / 64], Q6_V_vxor_VV(v_dup_m1, v_sign));
                HVX_Vector x10 = Q6_Vhf_vadd_VhfVhf(row_buffer0[c1 / 64], Q6_V_vxor_VV(v_dup_m0, v_sign));
                HVX_Vector x11 = Q6_Vhf_vadd_VhfVhf(row_buffer1[c1 / 64], Q6_V_vxor_VV(v_dup_m1, v_sign));
                const HVX_VectorPred q00 = Q6_Q_vcmp_gt_VhfVhf(row_buffer0[c0 / 64], v_neg_inf);
                const HVX_VectorPred q01 = Q6_Q_vcmp_gt_VhfVhf(row_buffer1[c0 / 64], v_neg_inf);
                const HVX_VectorPred q10 = Q6_Q_vcmp_gt_VhfVhf(row_buffer0[c1 / 64], v_neg_inf);
                const HVX_VectorPred q11 = Q6_Q_vcmp_gt_VhfVhf(row_buffer1[c1 / 64], v_neg_inf);
                x00 = Q6_V_vmux_QVV(q00, x00, v_scna_min);
                x01 = Q6_V_vmux_QVV(q01, x01, v_scna_min);
                x10 = Q6_V_vmux_QVV(q10, x10, v_scna_min);
                x11 = Q6_V_vmux_QVV(q11, x11, v_scna_min);
                HVX_Vector y00, y01, y10, y11;
                scna_d7_scalar_quad_inline(x00, x01, x10, x11, scna_hvx_params,
                                           &y00, &y01, &y10, &y11);
                y00 = Q6_V_vmux_QVV(q00, y00, v_zero_scna);
                y01 = Q6_V_vmux_QVV(q01, y01, v_zero_scna);
                y10 = Q6_V_vmux_QVV(q10, y10, v_zero_scna);
                y11 = Q6_V_vmux_QVV(q11, y11, v_zero_scna);
                SCNA_CONSUME_P_BLOCK(c0, y00, y01, row_buffer0[c0 / 64]);
                SCNA_CONSUME_P_BLOCK(c1, y10, y11, row_buffer0[c1 / 64]);
              }
            }
#endif
            int scna_c_end = n_cols;
#if STAGE2_SOFTMAX_IMPL == 1
            if (enable_scna_exp) {
              const HVX_Vector v_sign = Q6_Vh_vsplat_R(0x8000);
              HVX_Vector v_s_minus_m0 = Q6_Vhf_vadd_VhfVhf(v_s_tail0, Q6_V_vxor_VV(v_dup_m0, v_sign));
              HVX_Vector v_s_minus_m1 = Q6_Vhf_vadd_VhfVhf(v_s_tail1, Q6_V_vxor_VV(v_dup_m1, v_sign));
              const HVX_VectorPred q_valid0 = Q6_Q_vcmp_gt_VhfVhf(v_s_tail0, v_neg_inf);
              const HVX_VectorPred q_valid1 = Q6_Q_vcmp_gt_VhfVhf(v_s_tail1, v_neg_inf);
              const HVX_Vector v_zero_scna = Q6_V_vzero();
              __fp16 scna_min_hf = (__fp16) SCNA_MIN_INPUT;
              const HVX_Vector v_scna_min = Q6_Vh_vsplat_R(fp16_to_bits(&scna_min_hf));
              v_s_minus_m0 = Q6_V_vmux_QVV(q_valid0, v_s_minus_m0, v_scna_min);
              v_s_minus_m1 = Q6_V_vmux_QVV(q_valid1, v_s_minus_m1, v_scna_min);
              const HVX_VectorPair v_scna_pair =
                hvx_scna_exp2_pair_hot_return_vhf(v_s_minus_m0, v_s_minus_m1, scna_hvx_params);
              const HVX_Vector v_p_row0_hf = Q6_V_vmux_QVV(q_valid0, Q6_V_lo_W(v_scna_pair), v_zero_scna);
              const HVX_Vector v_p_row1_hf = Q6_V_vmux_QVV(q_valid1, Q6_V_hi_W(v_scna_pair), v_zero_scna);
              SCNA_CONSUME_P_BLOCK(scna_tail_c, v_p_row0_hf, v_p_row1_hf, v_s_tail0);
              scna_c_end = scna_tail_c;
            }
#endif
#if STAGE2_SOFTMAX_IMPL == 3
#pragma unroll 2
#endif
            for (int c = scna_c_begin; c < scna_c_end; c += 64) {
...
#if SCNA_KERNEL_IMPL == SCNA_KERNEL_IMPL_D7_PAIRRET_NOINLINE || \
    SCNA_KERNEL_IMPL == SCNA_KERNEL_IMPL_D7_PAIRRET_TWO_ACC || \
    SCNA_KERNEL_IMPL == SCNA_KERNEL_IMPL_D7_PAIRRET_SHORT_CONST || \
    SCNA_KERNEL_IMPL == SCNA_KERNEL_IMPL_D7_PAIRRET_TWO_NEURON || \
    SCNA_KERNEL_IMPL == SCNA_KERNEL_IMPL_D7_PAIRRET_SCALE_PROBE
                const HVX_VectorPair v_scna_pair =
                  hvx_scna_exp2_pair_hot_return_vhf(v_s_minus_m0, v_s_minus_m1, scna_hvx_params);
                v_scna_row0 = Q6_V_lo_W(v_scna_pair);
                v_scna_row1 = Q6_V_hi_W(v_scna_pair);
#elif SCNA_KERNEL_IMPL == SCNA_KERNEL_IMPL_D7_PAIRRET_INLINE || \
      SCNA_KERNEL_IMPL == SCNA_KERNEL_IMPL_COMBINED_CONFIRM
                const HVX_VectorPair v_scna_pair =
                  scna_d7_scalar_pair_inline(v_s_minus_m0, v_s_minus_m1, scna_hvx_params);
                v_scna_row0 = Q6_V_lo_W(v_scna_pair);
                v_scna_row1 = Q6_V_hi_W(v_scna_pair);
#elif SCNA_KERNEL_IMPL == SCNA_KERNEL_IMPL_D7_PREBROADCAST
                const HVX_VectorPair v_scna_pair =
                  scna_prebroadcast_pair_inline(v_s_minus_m0, v_s_minus_m1, &scna_prebroadcast);
                v_scna_row0 = Q6_V_lo_W(v_scna_pair);
                v_scna_row1 = Q6_V_hi_W(v_scna_pair);
#else
              ...
              SCNA_CONSUME_P_BLOCK(c, v_p_row0_hf, v_p_row1_hf, row_buffer0[c / 64]);
            }
#undef SCNA_CONSUME_P_BLOCK
            if (enable_scna_exp) {
...
#pragma unroll
            ...
            v_p_rowsum_local = Q6_V_vror_VR(v_p_rowsum_local_rot,
                                             (VLEN - r_vec_off * sizeof(__fp16)) % VLEN);
#if STAGE2_ENABLE_FINE_TIMERS
            TIMER_STOP(rowsum_reduce);
#endif
          }

          // Either materialize compact Softmax state for the later update or
          // consume it here while both rowmax and rowsum are still registers.
#if STAGE2_SOFTMAX_IMPL == 2 || STAGE2_SOFTMAX_IMPL == 3
#if STAGE2_ENABLE_FINE_TIMERS
          TIMER_START(state_update);
#endif
          HVX_Vector v_m_prev = mvec_m[r_vec_idx];
          HVX_Vector v_m_curr = Q6_Vhf_vmax_VhfVhf(v_m_prev, v_s_rowmax_local);
#if (STAGE25_CLEANUP_FLAGS & STAGE25_D_INVARIANT_HOIST) != 0
          const HVX_Vector v_state_sign = v_state_sign_hoisted;
#else
          const HVX_Vector v_state_sign = Q6_Vh_vsplat_R(0x8000);
#endif
          HVX_Vector v_m_diff_hf = Q6_Vhf_vadd_VhfVhf(v_m_prev, Q6_V_vxor_VV(v_m_curr, v_state_sign));
          ...
          if (enable_scna_exp) {
            ...
            figure8_profile_record_event(profile, FIGURE8_COMP_SCNA_EXP, 0, qo_len, kv_len, n_heads,
                                         n_kv_heads, head_dim, kv_head_idx, worker_index, ir + r_vec_idx * 64, jc,
          ...
          HVX_Vector v_l_curr = Q6_Vhf_vmpyacc_VhfVhfVhf(v_p_rowsum_local, mvec_l[r_vec_idx], v_exp_m_diff_hf);
          if (numeric_debug && kv_head_idx == 0 && ir == 0 && jc == 0 && r_vec_idx == 0) {
            _Alignas(VLEN) __fp16 debug_rowsum[64], debug_l[64];
            vmem(debug_rowsum) = v_p_rowsum_local;
            vmem(debug_l) = v_l_curr;
            debug_rowsum0_bits = fp16_to_bits(&debug_rowsum[0]);
            debug_l0_bits = fp16_to_bits(&debug_l[0]);
          }
          if (numeric_debug && kv_head_idx == 0 && ir == 0 && r_vec_idx == 0 && debug_block_count < 8) {
          ...
          mvec_m[r_vec_idx] = v_m_curr;
          mvec_l[r_vec_idx] = v_l_curr;
          mvec_s_rowmax[r_vec_idx] = v_exp_m_diff_hf;
#if STAGE2_ENABLE_FINE_TIMERS
          TIMER_STOP(state_update);
#endif
#else
...
#if (STAGE25_CLEANUP_FLAGS & STAGE25_A_MASK_INDEX) != 0
      int o_idx0_incremental = 0;
      int o_h_idx0_incremental = 0;
#endif
      for (int r = 0; r < n_rows_g; r += 2) {
        const bool next_row_valid = (r + 1) < n_rows_g;

#if (STAGE25_CLEANUP_FLAGS & STAGE25_A_MASK_INDEX) != 0
        int o_idx0 = o_idx0_incremental;
        int h_idx0 = o_h_idx0_incremental;
        int o_idx1 = o_idx0;
        int h_idx1 = h_idx0 + 1;
        if (h_idx1 >= G) {
          h_idx1 = 0;
        ...
        // clang-format on
#if (STAGE25_CLEANUP_FLAGS & STAGE25_A_MASK_INDEX) != 0
        o_idx0_incremental = o_idx1;
        o_h_idx0_incremental = h_idx1 + 1;
        if (o_h_idx0_incremental >= G) {
          o_h_idx0_incremental = 0;
          ++o_idx0_incremental;
        }
#endif
      }
...
#if defined(ENABLE_PROFILE_TIMERS)
  {
    ...
    const int64_t state_update_us = TIMER_US(state_update);
    const int64_t rowmax_mem_us = TIMER_US(rowmax_mem);
    const int64_t rowsum_reduce_us = TIMER_US(rowsum_reduce);
    ...
    FARF(ALWAYS,
         ...
         "o_scale=%lld o_store=%lld scna_exp=%lld state_update=%lld rowmax_mem=%lld rowsum_reduce=%lld",
         ...
         o_store_us, scna_exp_us, state_update_us, rowmax_mem_us, rowsum_reduce_us);
    ...
    if (profile != NULL) {
      ...
      if (idx >= 0 && idx < profile->max_records) {
        ...
        record[idx]                         = (struct Figure8ProfileRecord) {
          ...
          .state_update   = state_update_us,
          .rowmax_mem     = rowmax_mem_us,
          .rowsum_reduce  = rowsum_reduce_us,
          .rowmax_mem_ticks = rowmax_mem_ticks,
          .rowsum_reduce_ticks = rowsum_reduce_ticks,
          .state_update_ticks = state_update_ticks,
          .param_prepare  = enable_scna_exp ? scna_hvx_params->prepare_us : 0,
...
void simple_flash_attn_worker(void *data, int worker_index) {
  ...
  while (1) {
    ...
    const int q_chunk = task_id / s->n_kv_heads;
    const int kv_head_idx = task_id % s->n_kv_heads;
    const int q_begin = q_chunk * s->q_task_rows;
    const int q_count = smin(s->qo_len - q_begin, s->q_task_rows);
#if (STAGE25_CLEANUP_FLAGS & STAGE25_E_TASK_GEOMETRY) != 0
    const size_t q_stride = s->q_stride;
    const size_t mask_stride = s->mask_stride;
#else
    const size_t q_stride = (size_t) s->n_heads * s->head_dim;
    const size_t mask_stride = align_up(s->kv_len, 64);
#endif
    /* Q/O are stored as FP32 in this path despite their transport pointer
     * type.  Offset through float so a query-block task has correct strides. */
    __fp16 *o_task = (__fp16 *) ((float *) s->O + q_begin * q_stride);
    const __fp16 *q_task = (const __fp16 *) ((const float *) s->Q + q_begin * q_stride);
    const __fp16 *mask_task = s->mask + q_begin * mask_stride;
    simple_flash_attn_f16_core(kv_head_idx, vtcm, vtcm_limit, o_task, q_task, s->K, s->V, mask_task, q_count, s->kv_len,
                               s->n_heads, s->n_kv_heads, s->head_dim, worker_index, s->mode_flags,
                               &s->scna_hvx_params, s->profile,
...
static int simple_flash_attn_impl(__fp16 *restrict O, const __fp16 *restrict Q, const __fp16 *restrict K,
  ...
  const bool enable_scna_exp = scna_exp2_enabled(mode_flags);
  const int requested_variant = (mode_flags >> 10) & 7;
  if (enable_scna_exp && requested_variant != scna_exp2_build_variant()) {
    FARF(ALWAYS, "SCNA_BUILD_ID_MISMATCH requested=%d built=%d", requested_variant, scna_exp2_build_variant());
    return -2;
  }

  const int requested_workers = (mode_flags & LLM_NPU_MODE_WORKER_COUNT_MASK) >> LLM_NPU_MODE_WORKER_COUNT_SHIFT;
  const size_t vtcm_size_per_thread = 1024 * 1024;
  const int q_task_rows = 4;
  const int n_tasks = ceil_div(qo_len, q_task_rows) * n_kv_heads;
  const int vtcm_worker_cap = (int) (vtcm_manager_get_total_size() / vtcm_size_per_thread);
  const int requested_or_auto = requested_workers > 0 ? requested_workers : num_hvx128_contexts;
  const int n_workers = smax(1, smin(requested_or_auto,
      smin(num_hvx128_contexts, smin(n_tasks, vtcm_worker_cap))));

  if (profile != NULL && profile->magic == FIGURE8_PROFILE_MAGIC) {
    profile->active_workers = n_workers;
    profile->hvx_contexts = (int32_t) num_hvx128_contexts;
    profile->vtcm_worker_cap = vtcm_worker_cap;
    profile->tasks = n_tasks;
    profile->q_task_rows = q_task_rows;
  }
  ...
  /* The scheduler's unit is a fixed query block times one KV head.  This
   * exposes more than n_kv_heads tasks without altering the arithmetic. */
  state.q_task_rows = q_task_rows;
#if (STAGE25_CLEANUP_FLAGS & STAGE25_E_TASK_GEOMETRY) != 0
  state.q_stride = (size_t) n_heads * head_dim;
  state.mask_stride = align_up(kv_len, 64);
#else
  state.q_stride = 0;
  state.mask_stride = 0;
#endif
  state.mode_flags = mode_flags;
  state.scna_param_prepare_us = 0;
  memset(&state.scna_hvx_params, 0, sizeof(state.scna_hvx_params));
  if (enable_scna_exp) {
    const int64_t prepare_t0 = HAP_perf_get_qtimer_count();
    if (scna_exp2_prepare_hvx_params(&state.scna_hvx_params, mode_flags) != 0) return -3;
    state.scna_param_prepare_us = HAP_perf_qtimer_count_to_us(HAP_perf_get_qtimer_count() - prepare_t0);
    state.scna_hvx_params.prepare_us = state.scna_param_prepare_us;
  }
  ...
  FARF(ALWAYS,
       "FIG8_ATTENTION_KERNEL lut_exp=%d scna_build_variant=%d optimized_inline=%d param_prepare=%lld total_kernel=%lld qo_len=%d kv_len=%d n_heads=%d n_kv_heads=%d head_dim=%d requested_workers=%d active_workers=%d hvx_contexts=%d vtcm_worker_cap=%d tasks=%d q_task_rows=%d",
       (mode_flags & LLM_NPU_MODE_LUT_EXP) ? 1 : 0, scna_exp2_build_variant(),
       scna_exp2_build_optimized_inline(), state.scna_param_prepare_us, elapsed_us,
       qo_len, kv_len, n_heads, n_kv_heads, head_dim, requested_workers, n_workers,
       num_hvx128_contexts, vtcm_worker_cap, state.n_tasks, state.q_task_rows);
