...
static void figure8_print_usage(const char *prog) {
  fprintf(stderr,
          ...
          "          [--seed figure8_fixed|UINT32] [--fixture PATH] [--full-mask-fastpath] [--csv-out PATH]\n"
          "       %s --hmx-int8-gate\n"
  ...
  *cfg = (struct Figure8AttnConfig) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (strcmp(arg, "--figure8-attn") == 0) {
      ...
      if (++i >= argc) {
        fprintf(stderr, "Missing value for --fixture\n");
        return -1;
      }
      cfg->fixture_path = argv[i];
    } else if (strcmp(arg, "--full-mask-fastpath") == 0) {
      cfg->full_mask_fastpath = 1;
  ...
  if (cfg->full_mask_fastpath && strcmp(cfg->mask_mode, "full") != 0) {
    fprintf(stderr, "--full-mask-fastpath requires --mask-mode full\n");
    return -1;
  }
  if (cfg->scna_width != 8) {
...
_Static_assert(sizeof(struct Stage25FixtureHeader) == 80, "Stage 2.5 fixture header ABI");

static int stage25_mask_mode_id(const char *mask_mode) {
  if (strcmp(mask_mode, "full") == 0) return 0;
  if (strcmp(mask_mode, "causal") == 0) return 1;
  if (strcmp(mask_mode, "padding") == 0) return 2;
  return -1;
}

static int stage25_load_fixture(const struct Figure8AttnConfig *cfg, int kv_pad_len,
                                float *q, size_t q_bytes, __fp16 *k, size_t k_bytes,
                                __fp16 *v, size_t v_bytes, __fp16 *mask, size_t mask_bytes) {
  FILE *stream = fopen(cfg->fixture_path, "rb");
  if (!stream) {
    fprintf(stderr, "Failed to open fixture: %s\n", cfg->fixture_path);
    return -1;
  }
  struct Stage25FixtureHeader header;
  int ret = -1;
  if (fread(&header, sizeof(header), 1, stream) != 1) {
    fprintf(stderr, "Short fixture header: %s\n", cfg->fixture_path);
    goto end;
  }
  const char expected_magic[8] = { 'S', '2', '5', 'F', 'I', 'X', '0', '1' };
  if (memcmp(header.magic, expected_magic, sizeof(expected_magic)) != 0 || header.version != 1 ||
      header.qo_len != (uint32_t) cfg->qo_len || header.kv_len != (uint32_t) cfg->kv_len ||
      header.kv_pad_len != (uint32_t) kv_pad_len || header.n_heads != (uint32_t) cfg->n_heads ||
      header.n_kv_heads != (uint32_t) cfg->n_kv_heads || header.head_dim != (uint32_t) cfg->head_dim ||
      header.mask_mode != (uint32_t) stage25_mask_mode_id(cfg->mask_mode) ||
      header.q_bytes != q_bytes || header.k_bytes != k_bytes || header.v_bytes != v_bytes ||
      header.mask_bytes != mask_bytes) {
    fprintf(stderr, "Fixture contract mismatch: %s\n", cfg->fixture_path);
    goto end;
  }
  if (fread(q, q_bytes, 1, stream) != 1 || fread(k, k_bytes, 1, stream) != 1 ||
      fread(v, v_bytes, 1, stream) != 1 || fread(mask, mask_bytes, 1, stream) != 1 ||
      fgetc(stream) != EOF) {
    fprintf(stderr, "Fixture payload mismatch: %s\n", cfg->fixture_path);
    goto end;
  }
  ret = 0;
end:
...
static int run_figure8_attn_benchmark(const struct Figure8AttnConfig *cfg) {
  ...
  const size_t q_payload_size = (size_t) cfg->qo_len * cfg->n_heads * cfg->head_dim * sizeof(float);
  const size_t kv_payload_size = (size_t) cfg->kv_len * cfg->n_kv_heads * cfg->head_dim * sizeof(__fp16);
  const size_t mask_payload_size = (size_t) cfg->qo_len * kv_pad_len * sizeof(__fp16);
  if (cfg->fixture_path) {
    if (stage25_load_fixture(cfg, kv_pad_len, q, q_payload_size, k, kv_payload_size,
                             v, kv_payload_size, mask, mask_payload_size)) goto end;
  } else {
  ...
  if (cfg->csv_out) {
    ...
    fprintf(csv, "mode,variant,workers,qo_len,kv_len,n_heads,n_kv_heads,head_dim,phase,iteration,host_elapsed_us,ret\n");
  }
  ...
  if (cfg->full_mask_fastpath) mode_flags |= LLM_NPU_MODE_MASK_FULL_FASTPATH;
  ...
  fprintf(stderr,
          ...
          "profile_max_events=%d mode_flags=%d print_events=%d workers=%d fixture=%s full_mask_fastpath=%d\n",
          ...
          cfg->print_events, cfg->workers, cfg->fixture_path ? cfg->fixture_path : "generated",
          cfg->full_mask_fastpath);
  ...
  for (int i = 0; i < cfg->warmup + cfg->iters; ++i) {
    ...
    fprintf(stderr,
            "FIG8_ATTENTION_HOST_TIMING mode=%s layout=%s variant=%s workers=%d scna_width=%d seed=%s mask_mode=%s qo_len=%d kv_len=%d "
            "n_heads=%d n_kv_heads=%d head_dim=%d phase=%s "
            "iteration=%d host_elapsed_us=%ld ret=%d\n",
            cfg->mode, cfg->scna_layout, cfg->scna_variant, cfg->workers, cfg->scna_width, cfg->seed_label, cfg->mask_mode, cfg->qo_len, cfg->kv_len, cfg->n_heads,
            cfg->n_kv_heads, cfg->head_dim, phase,
            is_warmup ? i : measured_idx, elapsed, req_ret);
    if (csv) {
      fprintf(csv, "%s,%s,%d,%d,%d,%d,%d,%d,%s,%d,%ld,%d\n", cfg->mode, cfg->scna_variant, cfg->workers,
