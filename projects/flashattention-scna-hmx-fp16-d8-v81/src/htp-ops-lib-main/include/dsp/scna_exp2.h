...
static inline int scna_engine_from_mode(int mode_flags) {
  if ((mode_flags & LLM_NPU_MODE_SCNA_HMX_TWO_PASS) != 0) return SCNA_ENGINE_HMX_TWO_PASS;
  if ((mode_flags & LLM_NPU_MODE_SCNA_HMX_HYBRID) != 0) return SCNA_ENGINE_HMX_HYBRID;
  return SCNA_ENGINE_HVX;
}

static inline bool scna_hmx_enabled(int mode_flags) {
  return scna_engine_from_mode(mode_flags) != SCNA_ENGINE_HVX;
}
