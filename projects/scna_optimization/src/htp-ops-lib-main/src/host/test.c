...
static void figure8_print_usage(const char *prog) {
  fprintf(stderr,
          ...
          "       %s --scna-exp-bench [--warmup N] [--iters N]\n",
          prog,
          prog,
          prog,
          prog,
...
static int run_scna_exp_benchmark(const struct Figure8AttnConfig *cfg) {
  ...
  struct ScnaExp2BenchParams params = {
    ...
    .variant = SCNA_VARIANT_PAIR_STATIC_D8,
    .warmup = cfg->warmup,
    .iters = cfg->iters,
  };
...
static int run_figure8_attn_benchmark(const struct Figure8AttnConfig *cfg) {
  ...
  if (strcmp(cfg->mode, "scna-fp16") == 0) {
    ...
    mode_flags |= SCNA_VARIANT_PAIR_STATIC_D8 << 10;
  }
  mode_flags |= LLM_NPU_MODE_Q_TASK_ROWS_AUTO;
  ...
  if (cfg->compare_reference) {
    ...
    figure8_report_comparison(cfg, o, reference_output, output_elements);
  }
