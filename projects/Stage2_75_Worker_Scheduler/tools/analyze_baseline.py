#!/usr/bin/env python3
"""Analyze one self-contained Stage 2 three-path baseline run."""
...
LABELS = ("origin_hvx", "exp_lut", "d7_pairret_noinline")
QOS = (1, 4, 8, 16, 32)
HISTORICAL_US = {
    "origin_hvx": {1: 653.0, 4: 827.0, 8: 1568.5, 16: 2914.0, 32: 5613.5},
    "exp_lut": {1: 604.5, 4: 785.0, 8: 1337.5, 16: 2512.0, 32: 4708.0},
    "d7_pairret_noinline": {1: 659.5, 4: 782.0, 8: 1465.0, 16: 2767.0, 32: 5252.5},
}
...
def parse_micro(run_dir: Path) -> dict:
    result = {}
    for label in ("d7_pairret_noinline",):
        values = []
        checksums = set()
        ...
        for line in path.read_text(errors="replace").splitlines():
            ...
            if "FIG8_ATTENTION_TIMERS" in line and data.get("phase") == "measure":
                for key in ("q_load", "k_load", "v_load", "qk_dot", "safe_sm", "core_acc",
                            "o_scale", "o_store", "scna_exp", "state_update", "profiled_total"):
...
def render_report(summary: dict, report_path: Path, project: Path) -> None:
    ...
    main_q_drift = {
        q: attention["d7_pairret_noinline"][q]["median"] / HISTORICAL_US["d7_pairret_noinline"][q] - 1
        for q in (8, 16, 32) if attention["d7_pairret_noinline"][q]["median"] is not None
    }
    dynamic_drift_ok = len(main_q_drift) == 3 and all(abs(value) <= 0.15 for value in main_q_drift.values())
    baseline_ok = static_ok and correctness_ok and dynamic_drift_ok and not quick
    ...
    lines = [
        ...
        "## Decision",
        "",
        ("**PASS：三条 baseline 已同机、同参数重新测量，可以进入 Phase A。**" if baseline_ok else
        ...
        f"- Main-Q dynamic drift within preregistered ±15% audit band: `{'PASS' if dynamic_drift_ok else 'FAIL'}`",
        "",
        "## Question and hypothesis",
        "",
        "本实验检验全新独立构建的 `d7_pairret_noinline` 是否仍复现既有 correctness、micro、静态汇编和 Attention 行为，并在同一 carrier、同一设备、同一参数下重测 Origin HVX 与 EXP-LUT。历史延迟只用于漂移审计，不参与候选排名。",
        "",
    ...
    for label in ("origin_hvx", "exp_lut"):
    ...
    lines += [
        "",
        "Artifact SHA may differ from the historical run because this is a fresh isolated build; code-generation metrics are the primary static drift gate. The formal dynamic drift audit uses an explicit ±15% band on pairret q8/q16/q32 and never substitutes historical latency for fresh measurements.",
        "",
        "## Stage diagnostic",
        "",
        "|Version|Host us|QK|Softmax|SCNA|PV/core_acc|profiled total|",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for label in LABELS:
        row = summary["diagnostic"].get(label, {})
        lines.append(f"|{label}|{row.get('host_us')}|{row.get('qk_dot', 0):.0f}|{row.get('safe_sm', 0):.0f}|"
                     f"{row.get('scna_exp', 0):.0f}|{row.get('core_acc', 0):.0f}|{row.get('profiled_total', 0):.0f}|")
    lines += [
        "",
        "## Interpretation",
        "",
        ("静态签名、72-case correctness 与三条正式动态基线均已重新生成；后续实验只与本 run 比较。" if baseline_ok else
        ...
        f"- Recommended next experiment: `{'Phase A — reconstruct current Softmax dataflow' if baseline_ok else 'formal baseline reproduction' if quick else 'baseline drift diagnosis'}`",
        "",
