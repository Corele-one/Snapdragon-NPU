...
Deploys the FastRPC environment and the Stage 2 FlashAttention baseline carrier.

Options:
  --mode ping|baseline|lut-exp|scna-fp16|scna-exp-bench
  --dsp-arch v79                 DSP build target (this experiment is v79-only)
  --kernel-impl IMPL --workers auto|1..6 --q-task-rows auto|4|8|16|32 --scna-width 8
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --q-task-rows)
      [[ $# -ge 2 ]] || { usage; exit 2; }
      if [[ "$2" == auto ]]; then q_task_rows=auto; else require_positive_int "$1" "$2"; q_task_rows="$2"; fi
      shift 2 ;;
    --scna-width)
      [[ $# -ge 2 ]] || { usage; exit 2; }
...
[[ "$q_task_rows" == auto || "$q_task_rows" == 4 || "$q_task_rows" == 8 || "$q_task_rows" == 16 || "$q_task_rows" == 32 ]] || {
  echo "--q-task-rows must be auto or one of 4, 8, 16, or 32" >&2; exit 2;
}
case "$kernel_impl" in
...
[[ -f "$variant_artifact" ]] || {
  echo "Missing independent DSP artifact: $variant_artifact. Run scripts/build_baselines.sh first." >&2
  exit 1
}
...
if [[ "$mode" == "ping" ]]; then
  ...
  adb shell "cd '$remote_dir' && LD_LIBRARY_PATH=. DSP_LIBRARY_PATH='./cdsp;./dsp;.' ./htp_ops_test --figure8-attn --mode '$mode' --scna-variant pair_static_d8 --workers '$workers' --q-task-rows '$q_task_rows' --scna-width '$scna_width' --mask-mode '$mask_mode' --qo-len '$qo_len' --kv-len '$kv_len' --n-heads '$n_heads' --n-kv-heads '$n_kv_heads' --head-dim '$head_dim' --warmup '$warmup' --iters '$iters' $event_arg $reference_arg"
fi
