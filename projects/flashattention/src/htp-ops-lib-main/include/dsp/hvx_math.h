...
/* v79+ XQF range reduction from Qualcomm's qhmath HVX exp2 path. The v73
 * build keeps the original implementation because these intrinsics are not
 * part of its compatibility contract. */
static HVX_INLINE_ALWAYS HVX_Vector hvx_my_exp2_xqf_vhf(HVX_Vector input) {
#if __HVX_ARCH__ >= 79
...
/* Direct FP16 subtraction is available on the v79+ path. Older targets keep
 * the original qf16 subtraction and explicitly round back to FP16. */
static HVX_INLINE_ALWAYS HVX_Vector hvx_attention_sub_vhf(HVX_Vector lhs, HVX_Vector rhs) {
#if __HVX_ARCH__ >= 79
  const HVX_Vector v_sign = Q6_Vh_vsplat_R(0x8000);
  return Q6_Vhf_vadd_VhfVhf(lhs, Q6_V_vxor_VV(rhs, v_sign));
#else
  return Q6_Vhf_equals_Vqf16(Q6_Vqf16_vsub_VhfVhf(lhs, rhs));
#endif
}
