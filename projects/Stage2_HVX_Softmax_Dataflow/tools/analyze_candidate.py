#!/usr/bin/env python3
"""Analyze a paired Stage 2 Softmax candidate run into auditable JSON."""
...
def parse_attention(run_dir: Path, candidate: str) -> tuple[dict, dict]:
    labels = (ORIGIN, LUT, PAIRRET, candidate)
...
def parse_diagnostics(run_dir: Path, candidate: str) -> dict:
    result = {}
    for label in (ORIGIN, LUT, PAIRRET, candidate):
...
def main() -> None:
    ...
    baseline_artifact = manifest.get("baseline_artifact", PAIRRET)
    ...
    summary = {
        ...
        "artifact_sha256": {
            PAIRRET: sha256(project / f"artifacts/variants/{baseline_artifact}/libhtp_ops_skel.so"),
