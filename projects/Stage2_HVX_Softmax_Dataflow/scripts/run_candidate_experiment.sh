...
candidate="tail_s_resident"
baseline_artifact="d7_pairret_noinline"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --quick) quick=1; shift ;;
    --help|-h)
      echo "Usage: $0 [--candidate IMPL] [--baseline-artifact NAME] [--run-id ID] [--remote-dir PATH] [--quick]"
      exit 0 ;;
...
case "$candidate" in
  tail_s_resident|fused_state_update|fused_state_unroll2) ;;
...
pairret="$project_dir/artifacts/variants/$baseline_artifact/libhtp_ops_skel.so"
...
labels=(origin_hvx exp_lut d7_pairret_noinline "$candidate")
qos=(1 4 8 16 32)
